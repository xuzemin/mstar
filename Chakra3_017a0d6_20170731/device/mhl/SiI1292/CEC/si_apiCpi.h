...
typedef struct
{
    uint8_si srcDestAddr;            // Source in upper nybble, dest in lower nybble
    uint8_si opcode;
    uint8_si args[ SII_MAX_CMD_SIZE ];
    uint8_si argCount;
   	uint8_si nextFrameArgCount;
} SI_CpiData_t;
...
bool_si SI_CpiStatus( SI_CpiStatus_t *pCpiStat );
