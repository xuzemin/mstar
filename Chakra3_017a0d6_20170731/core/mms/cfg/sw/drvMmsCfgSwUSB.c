...
void MDrv_Cfg_SwUSB(void)
{
   MDrv_UsbUpdate();
}

void MDrv_Cfg_SwUSBEntry(void)
{
    cfg_level = USB;
    dynLevel = 1;
...
void MDrv_Cfg_SwUSBPrint(void)
{
  printf("Execute USB update\n");
  //printf(":\\M\\cfg\\sw\\USB>");
}

void MDrv_Cfg_SwUSBEntryPrint(void)
{
  printf("Enter USB page\n");
}


void MDrv_Cfg_SWUSBInit(void)
{
    //(1)main page list
    MDrv_Cfg_Act(SW, SW_MAIN, MDrv_Cfg_SwUSBEntry, MDrv_Cfg_SwUSBEntryPrint);

    //(2)Action function(s)
    MDrv_Cfg_Act(SW, USB, MDrv_Cfg_SwUSB, MDrv_Cfg_SwUSBPrint);

    //(3)jump to Driver main page
    MDrv_Cfg_Act(SW, USB, MDrv_CfgGotoSWMainPage, MDrv_CfgGotoSWMainPagePrint);

    //(4)jump to Debug system main page
    MDrv_Cfg_Act(SW, USB, MDrv_CfgGotoCfgMainPage, MDrv_CfgGotoCfgMainPagePrint);

}

void MDrv_UsbUpdate(void)
{
#if ( (ENABLE_USB||ENABLE_USB_2||ENABLE_USB_3||ENABLE_USB_4) && ENABLE_FILESYSTEM && (MS_SW_CUS_SEL == SW_CONFIG_TRUNK_DVBT))
#ifndef MSOS_TYPE_LINUX
                extern U8 MDrv_USBGetPortEnableStatus(void);
                extern void MApp_ZUI_SwUpdate_ProgressBar(U8 percent);
                U8 u8PortEnStatus = 0;
                printf("USB SW Update(Menu mode) ...\n");
