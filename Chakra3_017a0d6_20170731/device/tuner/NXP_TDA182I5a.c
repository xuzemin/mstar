...
* Class : device_tuner_NXP_TDA182I5a
* File  : device_tuner_NXP_TDA182I5a.cpp
**********************************************************************/
...
#include <unistd.h>
#include "debug.h"
#include "device_tuner_NXP_TDA182I5a.h"
#include "mapi_i2c.h"
...
#include "NXP_TDA182I5a/tmFlags.h"
#include "NXP_TDA182I5a/tmbslTDA182I5a_RegDef.c"
#include "NXP_TDA182I5a/tmbslTDA182I5a_Instance.c"
#include "NXP_TDA182I5a/tmbslTDA182I5a.c"
...
#if 0
// tmbslTDA182I5a_InstanceCustom.h
// Digital IF
#define QAM_6M     4000L
#define QAM_8M     5000L
#define ATSC_6M    5000L//5250L
...
#define ATV_VC_MN  6000L //5400L
#define ATV_VC_B   7000L //6250L
#define ATV_VC_GH  7000L //6750L
#define ATV_VC_I   7500L //7250L
#define ATV_VC_DK  8000L //6850L
#define ATV_VC_L   8000L //6750L
...
#endif
#define NXP182I5a_I2C_ADDR (((U16)TUNER_IIC_BUS<<8)| _TUNER_I2C_ID)

/*@ </Definitions> @*/

#define TM_FALSE  1
#define TDA182I5a_BUSY  1
#define TDA182I5a_FREE  0
// static MS_U8 u8TDA182I5aBusy=TDA182I5a_FREE;
static MS_U8 bInitNXP_TDA182I5a = 0;
// pthread_mutex_t m_mutexTunerInit = PTHREAD_MUTEX_INITIALIZER;

void devTDA182I5aTunerInit(void);
...
// #define ATV_VC_I   7500L, R = 0.9
const MS_U16 VIF_A1_A2_SOS11_SOS12_I[16]=
{
 0x0384,0x0661,0x0418,
 0x0391,0x0661,0x040A,
 0x00AD,0x0661,0x0200,0x0740,0x0200,
 0x0384,0x0661,0x0200,0x0418,0x0200
};
...
{
    device_tuner_NXP_TDA182I5a* _this =(device_tuner_NXP_TDA182I5a*)ptr;
    printf("Tuner Init Thread\n");
    if (_this->m_bDeviceBusy == 1)
        return FALSE;

    _this->m_bDeviceBusy = 1;
    if(!bInitNXP_TDA182I5a) //double checklock
    {
        mapi_scope_lock(scopeLock, &m_mutexTunerInit);
        if(!bInitNXP_TDA182I5a)
        {
            devTDA182I5aTunerInit();
            bInitNXP_TDA182I5a = 1;
        }
    }
...
device_tuner_NXP_TDA182I5a(void)
{
    m_bDeviceBusy = 0;
}
...
{
    printf("device_tuner_NXP_TDA182I5a: Connect\n");
    if (m_bDeviceBusy == 1)
        return FALSE;
...
BOOLEAN Disconnect()
{
    printf("device_tuner_NXP_TDA182I5a: Disconnect\n");
...
UInt32 ReadLen, UInt8* pData)
{
    ...
    UNUSED(tUnit);
    if(MDrv_IIC_ReadBytes(NXP182I5a_I2C_ADDR, (U8) AddrSize, pAddr, (U16) ReadLen, pData))
	{
	    err = TM_OK;
	/*
...
UInt32 WriteLen, UInt8* pData)
{
	...
	printf("\n");
	*/
    if(MDrv_IIC_WriteBytes(NXP182I5a_I2C_ADDR, (U8) AddrSize, pAddr, (U16) WriteLen, pData))
    {
	    err = TM_OK;
    }
    else 
...
{     
...
#if 0    
    tmTDA182I5aStandardMode_t stdMode = tmTDA182I5a_ANLG_B;
    UInt32 uRF = 55250000;
    tmbslFrontEndState_t PLLLock = tmbslFrontEndStateUnknown;
    UInt32 uIF = 0;
#endif

    if (bInitNXP_TDA182I5a == 1) 
    {
        printf("%s, %d, warnning\n",__FILE__,__LINE__);
        return;
    }
    ...
    err = tmbslTDA182I5a_Open(0, &sSrvTunerFunc);
    
    if(err == TM_OK)
    {   
         /* TDA182I5a Hardware initialization */
         err = tmbslTDA182I5a_HwInit(0);
    }
#if 0    
    if(err == TM_OK)
    {
         /* TDA182I5a standard mode selection */
         err = tmbslTDA182I5a_SetStandardMode(0, stdMode);
    }
    if(err == TM_OK)
    {
         /* TDA182I5a RF frequency selection */
         err = tmbslTDA182I5a_SetRF(0, uRF);
    }
    if(err == TM_OK)
    {
         /* Get TDA182I5a PLL Lock status */
         err = tmbslTDA182I5a_GetLockStatus(0, &PLLLock);
    }
    if(err == TM_OK)
    {
         /* Get TDA182I5a IF value */
         err = tmbslTDA182I5a_GetIF(0, &uIF); 
    }
    if(err == TM_OK)
    {
...
void devTDA182I5aDigitalTuner_Init(void)
{
    devTDA182I5aTunerInit();
}
...
{    
    tmTDA182I5aStandardMode_t StandardMode = tmTDA182I5a_ANLG_B;
    tmErrorCode_t   err = TM_OK;
    UNUSED(eBand);
    ...
    if(!bInitNXP_TDA182I5a) //double check lock
    {
        devTDA182I5aTunerInit();
        if (bInitNXP_TDA182I5a == 0)
        {
            return FALSE;
        }
    }
    ...
    if(eMode == E_TUNER_ATV_SECAM_L_PRIME_MODE)
    {
        ...
        StandardMode = tmTDA182I5a_ANLG_LL;
    }
    else if(eMode == E_TUNER_ATV_NTSC_MODE)
    {
        StandardMode = tmTDA182I5a_ANLG_MN;
    }
    else
    {
        ...
        if(u8OtherMode == 0xff)
        {
            StandardMode=tmTDA182I5a_ANLG_DK;
        }
        else
        {
            ...
            case E_AUDIOSTANDARD_BG_:
                StandardMode=tmTDA182I5a_ANLG_B;
                break;

            case E_AUDIOSTANDARD_I_:
                StandardMode=tmTDA182I5a_ANLG_I;
                break;
            ...
            case E_AUDIOSTANDARD_L_:
                StandardMode=tmTDA182I5a_ANLG_L;
                break;
    ...
    {
         /* TDA182I5a standard mode selection */
         err = tmbslTDA182I5a_SetStandardMode(0, StandardMode);
    }
    if(err == TM_OK)
    {
         /* TDA182I5a RF frequency selection */
...
{
    tmTDA182I5aStandardMode_t StandardMode = tmTDA182I5a_DVBT_8MHz;
    tmbslFrontEndState_t PLLLock = tmbslFrontEndStateUnknown;
    UInt32          uIF = 0;
    tmErrorCode_t   err = TM_OK;    
    ...
    if (eMode == E_TUNER_DTV_ATSC_MODE)
        uRF = (UInt32)(Freq * 1000.0);
    else
        uRF = (UInt32)(Freq * 1000000.0);

    if(!bInitNXP_TDA182I5a)
    {
        devTDA182I5aDigitalTuner_Init();
        if (bInitNXP_TDA182I5a == 0)
        {
            return FALSE;
        }
    ...
    DBG_TUNER(printf("%s, %d, FreqKhz=%f,bw=%d,eMode=%d,uRF=%ld\n",__FILE__,__LINE__,Freq,eBandWidth,eMode,uRF);)

    switch (eMode)
    {
        case E_TUNER_DTV_DVB_C_MODE:
            printf("\r\nSilicon  Tuner mode DVBC\n");
            StandardMode = tmTDA182I5a_QAM_8MHz;
            break;
        ...
        case E_TUNER_DTV_DVB_T2_MODE:
            {
                ...
                switch (eBandWidth)
                {
                    case E_RF_CH_BAND_6MHz:
                        StandardMode = tmTDA182I5a_DVBT_6MHz;
                        break;
                    case E_RF_CH_BAND_7MHz:
                        StandardMode = tmTDA182I5a_DVBT_7MHz;
                        break;
                    case E_RF_CH_BAND_8MHz:
                    default:
                        StandardMode = tmTDA182I5a_DVBT_8MHz;
                        break;
                }
            }
            break;
        case E_TUNER_DTV_DTMB_MODE:
            StandardMode = tmTDA182I5a_DMBT_8MHz;
            break;
        case E_TUNER_DTV_ATSC_MODE:
            StandardMode = tmTDA182I5a_ATSC_6MHz;
            break;
        case E_TUNER_DTV_ISDB_MODE:
            StandardMode = tmTDA182I5a_ISDBT_6MHz;
            break;
        default:
            StandardMode = tmTDA182I5a_DVBT_8MHz;
...
BOOLEAN mdev_GetSignalStrength(float* f_strength)
{
    ...
    err = tmbslTDA182I5a_GetPowerLevel(0, &u8_rf_level_1_2_dubV);
    
    if(err != TM_OK)
    {
        rssi = -10.0f;
        printf("NuTune TDA182I5a RF Level Get Error\n");
    }
    else
    {               
        rssi = (float)u8_rf_level_1_2_dubV/2.0-107.0f; // dBuV to dBm
        DBG_TUNER(printf("NXP TDA182I5a RF Level %f dBm, %d(1/2dbuV)\n",rssi,u8_rf_level_1_2_dubV);)
    }
    *f_strength = rssi;
    return err;
}
...
BOOLEAN ExtendCommand(MS_U8 u8SubCmd, MS_U32 u32Param1, MS_U32 u32Param2, void* pvoidParam3)
{
    ...
    switch(u8SubCmd)
    {
        ...
        case mapi_tuner_datatype::E_TUNER_SUBCMD_GET_VIF_NOTCH_SOSFILTER:
        {
            ...
            {
            	// refine notch filter 2014.2.12 for at least 1.5MHz frequency spacing between audio carrier and DC
                case mapi_vif_datatype::E_VIF_SOUND_B_STEREO_A2: // Vf = 7MHz, R = 0.96
                    pu16NotchData[0] = 0x03C0;   //Vif_N_A1_C0
                    pu16NotchData[1] = 0x0628;   //Vif_N_A1_C1
                    pu16NotchData[2] = 0x0418;   //Vif_N_A1_C2

                    pu16NotchData[3] = 0x03C7;   //Vif_N_A2_C0
                    pu16NotchData[4] = 0x0628;   //Vif_N_A2_C1
                    pu16NotchData[5] = 0x0411;   //Vif_N_A2_C2

                    pu16SOSFilterData[0] = 0x0143;   //Vif_SOS_11_C0
                    pu16SOSFilterData[1] = 0x0628;   //Vif_SOS_11_C1
                    pu16SOSFilterData[2] = 0x0200;   //Vif_SOS_11_C2
                    pu16SOSFilterData[3] = 0x06B0;   //Vif_SOS_11_C3
                    pu16SOSFilterData[4] = 0x0200;   //Vif_SOS_11_C4

                    pu16SOSFilterData[5] = 0x03C0;   //Vif_SOS_12_C0
                    pu16SOSFilterData[6] = 0x0628;   //Vif_SOS_12_C1
                    pu16SOSFilterData[7] = 0x0200;   //Vif_SOS_12_C2
                    pu16SOSFilterData[8] = 0x0418;   //Vif_SOS_12_C3
                    pu16SOSFilterData[9] = 0x0200;   //Vif_SOS_12_C4
                    break;

                case mapi_vif_datatype::E_VIF_SOUND_B_MONO_NICAM: // Vf = 7MHz, R = 0.96
                    pu16NotchData[0] = 0x03C0;
                    pu16NotchData[1] = 0x0628;
                    pu16NotchData[2] = 0x0418;

                    pu16NotchData[3] = 0x03C9;
                    pu16NotchData[4] = 0x0628;
                    pu16NotchData[5] = 0x040E;

                    pu16SOSFilterData[0] = 0x0143;
                    pu16SOSFilterData[1] = 0x0628;
                    pu16SOSFilterData[2] = 0x0200;
                    pu16SOSFilterData[3] = 0x06B0;
                    pu16SOSFilterData[4] = 0x0200;

                    pu16SOSFilterData[5] = 0x03C0;
                    pu16SOSFilterData[6] = 0x0628;
                    pu16SOSFilterData[7] = 0x0200;
                    pu16SOSFilterData[8] = 0x0418;
                    pu16SOSFilterData[9] = 0x0200;
                    break;

                case mapi_vif_datatype::E_VIF_SOUND_GH_STEREO_A2: // Vf = 7MHz, R = 0.96
                    pu16NotchData[0] = 0x03C0;
                    pu16NotchData[1] = 0x0628;
                    pu16NotchData[2] = 0x0418;

                    pu16NotchData[3] = 0x03C7;
                    pu16NotchData[4] = 0x0628;
                    pu16NotchData[5] = 0x0411;

                    pu16SOSFilterData[0] = 0x00B9;
                    pu16SOSFilterData[1] = 0x0628;
                    pu16SOSFilterData[2] = 0x0200;
                    pu16SOSFilterData[3] = 0x0740;
                    pu16SOSFilterData[4] = 0x0200;
                ...
                case mapi_vif_datatype::E_VIF_SOUND_GH_MONO_NICAM:// Vf = 7MHz, R = 0.96
                    pu16NotchData[0] = 0x03C0;
                    pu16NotchData[1] = 0x0628;
                    pu16NotchData[2] = 0x0418;
                ...
                case mapi_vif_datatype::E_VIF_SOUND_I: // Vf = 7.5MHz, R = 0.96
                    pu16NotchData[0] = 0x03C0;//0x03C7;
                    pu16NotchData[1] = 0x0628;//0x0628;
                    pu16NotchData[2] = 0x0418;//0x0411;

                    pu16NotchData[3] = 0x03CE;//0x03D2;
                    pu16NotchData[4] = 0x0628;//0x0628;
                    pu16NotchData[5] = 0x040A;//0x0405;

                    pu16SOSFilterData[0] = 0x00B9;//0x00DC;
                    pu16SOSFilterData[1] = 0x0628;//0x0628;
                    pu16SOSFilterData[2] = 0x0200;//0x0200;
                    pu16SOSFilterData[3] = 0x0740;//0x071B;
                    pu16SOSFilterData[4] = 0x0200;//0x0200;

                    pu16SOSFilterData[5] = 0x03C0;//0x03C7;
                    pu16SOSFilterData[6] = 0x0628;//0x0628;
                    pu16SOSFilterData[7] = 0x0200;//0x0200;
                    pu16SOSFilterData[8] = 0x0418;//0x0411;
                    pu16SOSFilterData[9] = 0x0200;//0x0200;
                    break;

                case mapi_vif_datatype::E_VIF_SOUND_DK1_STEREO_A2:// Vf = 8MHz, R = 0.96
                    pu16NotchData[0] = 0x03C0;
                    pu16NotchData[1] = 0x0628;
                    pu16NotchData[2] = 0x0418;

                    pu16NotchData[3] = 0x03B8;
                    pu16NotchData[4] = 0x0628;
                    pu16NotchData[5] = 0x0421;
                ...
                case mapi_vif_datatype::E_VIF_SOUND_DK2_STEREO_A2:// Vf = 8MHz, R = 0.96
                    pu16NotchData[0] = 0x03C0;
                    pu16NotchData[1] = 0x0628;
                    pu16NotchData[2] = 0x0418;
                ...
                case mapi_vif_datatype::E_VIF_SOUND_DK3_STEREO_A2:// Vf = 8MHz, R = 0.96
                    pu16NotchData[0] = 0x03C0;
                    pu16NotchData[1] = 0x0628;
                    pu16NotchData[2] = 0x0418;

                    pu16NotchData[3] = 0x03A2;
                    pu16NotchData[4] = 0x0628;
                    pu16NotchData[5] = 0x0437;
                ...
                case mapi_vif_datatype::E_VIF_SOUND_DK_MONO_NICAM:// Vf = 8MHz, R = 0.96
                    pu16NotchData[0] = 0x03C0;
                    pu16NotchData[1] = 0x0628;
                    pu16NotchData[2] = 0x0418;

                    pu16NotchData[3] = 0x03A7;
                    pu16NotchData[4] = 0x0628;
                    pu16NotchData[5] = 0x0432;
                ...
                case mapi_vif_datatype::E_VIF_SOUND_L:// Vf = 8MHz, R = 0.96
                    pu16NotchData[0] = 0x03C0;
                    pu16NotchData[1] = 0x0628;
                    pu16NotchData[2] = 0x0418;
                ...
                case mapi_vif_datatype::E_VIF_SOUND_LL: //no change
                ...
                case mapi_vif_datatype::E_VIF_SOUND_MN: // Vf = 6MHz, R = 0.96
                    pu16NotchData[0] = 0x03C0;
                    pu16NotchData[1] = 0x0628;
                    pu16NotchData[2] = 0x0418;

                    pu16NotchData[3] = 0x03C6;
                    pu16NotchData[4] = 0x0628;
                    pu16NotchData[5] = 0x0412;

                    pu16SOSFilterData[0] = 0x01C6;
                    pu16SOSFilterData[1] = 0x0628;
                    pu16SOSFilterData[2] = 0x0200;
                    pu16SOSFilterData[3] = 0x0627;
                    pu16SOSFilterData[4] = 0x0200;
