...
#define DTMB_TS_CLK_INV      1 //TS_CLK_INV  // TS clock inverseion //this define will depend on hw main board
...
*/
BOOLEAN EXTERN_DTMB_Power_On_Initialization(void)
{
    ...
    printf("EXTERN_DTMB_Power_On_Initialization\n");
	...
	sDMD_DTMB_InitData.bIsUseSspiLoadCode=0;
	sDMD_DTMB_InitData.bIsSspiUseTsPin=0;
    ...
    sDMD_DTMB_InitData.bIsExtDemod=1;//0
  ...
  #if (FRONTEND_DEMOD_DTMB_TYPE == MSTAR_MSB101M_DTMB_MSD046B)
    MDrv_DMD_DTMB_DoIQSwap(1);
  #endif

    return bResult;
}

BOOLEAN EXTERN_DTMB_Lock(COFDM_LOCK_STATUS eStatus)
{
...
BOOLEAN EXTERN_DTMB_Config(RF_CHANNEL_BANDWIDTH BW, BOOLEAN bSerialTS, BOOLEAN bPalBG, BOOLEAN bLPSel)
{
    BW = BW;
    bSerialTS = bSerialTS;
    bPalBG = bPalBG;
...
BOOLEAN EXTERN_DTMB_Active(BOOLEAN bEnable)
{
    return MDrv_DMD_DTMB_SetConfig(DMD_DTMB_DEMOD_DTMB,bEnable);
}

BOOLEAN EXTERN_DTMB_Serial_Control(BOOLEAN bEnable)
{
...
FUNCTION_RESULT EXTERN_DTMB_GetSignalStrength(WORD *strength)
{
...
FUNCTION_RESULT EXTERN_DTMB_GetSignalQuality(WORD *quality)
{
    FUNCTION_RESULT ret = E_RESULT_SUCCESS;

    *quality = MDrv_DMD_DTMB_GetSignalQuality();
    //printf("EXTERN_DTMB_GetSignalQuality = %ld\n",  *quality);
    if ( *quality > 100 )
        ret = E_RESULT_FAILURE;
    return ret;
}
BOOLEAN EXTERN_DTMB_WriteReg(MS_U16 RegAddr, MS_U8 u8Data)
{
    return MDrv_DMD_DTMB_SetReg(RegAddr, u8Data);
}
BOOLEAN EXTERN_DTMB_ReadReg(MS_U16 RegAddr, MS_U8* pu8Data)
{
    return MDrv_DMD_DTMB_GetReg(RegAddr, pu8Data);
}

BOOLEAN EXTERN_DTMB_Exit ( void )
{
    return MDrv_DMD_DTMB_Exit();
}

void EXTERN_DTMB_Power_ON_OFF(MS_U8 bPowerOn)
{
    bPowerOn = bPowerOn;
}

BOOLEAN EXTERN_DTMB_Power_Save(void)
{
...
/************************************************************************************************
  ...
  Function:   EXTERN_DTMB_PAD_TS1_Enable
  Parmeter:   flag : TRUE For Turn on TS1, FALSE For Turn off TS1
  Return:     void
  Remark:
*************************************************************************************************/
void EXTERN_DTMB_PAD_TS1_Enable(BOOLEAN flag)
{
    printf(" @EXTERN_DTMB_PAD_TS1_Enable... \n");
