...
R840_ErrCode R840_InitReg(void)
{
	...
	if(R840_I2C_Write_Len(&R840_I2C_Len) != RT_Success)
		return RT_Fail;
	...
	if(R840_I2C_Write(&R840_I2C) != RT_Success)
	    return RT_Fail;
...
R840_ErrCode R840_TF_Check(void)
{
	...
	R840_I2C_Len.RegAddr = 0x00;
	R840_I2C_Len.Len = 4;
	if(R840_I2C_Read_Len(&R840_I2C_Len) != RT_Success)
	{
	    R840_I2C_Read_Len(&R840_I2C_Len);
	}	
...
if(R840_Chip==R840_R)
{
	...
	if(R840_IMR_done_flag == TRUE)
	{
		...
		if(((Reg08_IMR_Gain & 0x0F)>6) || ((Reg09_IMR_Phase & 0x0F)>6))
		{
			Reg08_IMR_Gain = 0;
			Reg09_IMR_Phase = 0;
			Reg03_IMR_Iqcap = 0;
...
R840_ErrCode R840_Standby(R840_LoopThrough_Type R840_LoopSwitch)
{
	...
	//Manual Gain
	//LNA auto off
	if((R840_Chip==R836_R) || (R840_Chip==R840_R))
	{
     R840_I2C.RegAddr = 0x0F;
	...
	//set LNA gain
	 ...
	 //set Byp_lpf=bypass
	 R840_I2C.RegAddr = 0x01;
	 R840_Array[1] = (R840_Array[1] & 0xFD) | 0x00; //R1[2]=0
	 R840_I2C.Data = R840_Array[1];
	 if(R840_I2C_Write(&R840_I2C) != RT_Success)
		 return RT_Fail;
...
R840_ErrCode R840_GetIfRssi(INT32 *VgaGain)
{
	...
	if(R840_I2C_Write(&Dlg_I2C) != RT_Success)
		return RT_Fail;
	...
	Dlg_I2C_Len.Len = 2;
	if(R840_I2C_Read_Len(&Dlg_I2C_Len) != RT_Success) // read data length
	{
		R840_I2C_Read_Len(&Dlg_I2C_Len);
	}
...
UINT8 R840_Chip_Detect(void)
{
	I2C_LEN_TYPE Dlg_I2C_Len;
	Dlg_I2C_Len.RegAddr = 0;
	Dlg_I2C_Len.Len = 1;
	memset(Dlg_I2C_Len.Data,0,50);
	if(R840_I2C_Read_Len(&Dlg_I2C_Len))
	{
		if(Dlg_I2C_Len.Data[0]== R840_CHIP_ID)
			return R840_CHIP_ID;
	}
	return 0xFF;
}
...
int R840_Convert(int InvertNum)
{
	int ReturnNum = 0;
	int AddNum    = 0x80;
	int BitNum    = 0x01;
...
bool R840_I2C_Write_Len(I2C_LEN_TYPE *I2C_Info)
{
	int i = 0,count;
	...
	if(MDrv_IIC_WriteBytes(R840_ADDRESS, 1, &regAddr, count, i2cData)==FALSE)
	{
		printf("\t ---->>RF R840 iic failed[%d]\n", regAddr);
		WRITE_OK = FALSE;
	}
	else
	{
...
bool R840_I2C_Read_Len(I2C_LEN_TYPE *I2C_Info)
{
	int i = 0,count;
	unsigned char regAddr;
	unsigned char i2cData[50];
    ...
    if ( MDrv_IIC_ReadBytes(R840_ADDRESS, 1, &regAddr,count, i2cData) == FALSE )
    {
		printf("\t ---->>Read RF R840 iic failed[%d]\n", regAddr);
		READ_OK = FALSE;
    }

	for(i = 0; i < count; i ++)
	{
		I2C_Info->Data[i] = R840_Convert(i2cData[i]);
	}
	return READ_OK;
}
...
bool R840_I2C_Write(I2C_TYPE *I2C_Info)
{
	unsigned char regAddr;
	...
	if(MDrv_IIC_WriteBytes(R840_ADDRESS, 1, &regAddr, 1, &i2cData)==FALSE)
	{
		printf("\t ---->>RF R840 iic failed[%d]\n", regAddr);
		WRITE_OK = FALSE;
	}
...
BOOLEAN R840_devTunerInit(void)
{
    R840_ErrCode ret;
	ret = R840_Init();
	if(ret == RT_Success)
