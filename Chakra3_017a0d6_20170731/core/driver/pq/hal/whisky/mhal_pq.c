...
#include "Whisky_Main.h"              // table config parameter
...
void Hal_PQ_set_y_sram_table(MS_U8 u8sramtype, void *pData)
{
    ...
    for ( i=0; i<64; i++)
    {
        ...
        if(u8sramtype == SC_FILTER_Y_SRAM1)
            MApi_XC_W2BYTEMSK(REG_SC_BK23_42_L, (0x0000|i), 0x07F);
        else if(u8sramtype == SC_FILTER_Y_SRAM2)
            MApi_XC_W2BYTEMSK(REG_SC_BK23_42_L, (0x0040|i), 0x07F);
...
void _Hal_PQ_set_sram_icc_crd_table(MS_U16 *pBuf, MS_U8 u8SRAM_Idx, MS_U16 u16Cnt)
{
    MS_U16 i;
...
{
    MS_U16 SRAM1_ICC[ICC_MAX_SRAM_SIZE];
    MS_U16 SRAM2_ICC[ICC_MAX_SRAM_SIZE];
    MS_U16 SRAM3_ICC[ICC_MAX_SRAM_SIZE];
    MS_U16 SRAM4_ICC[ICC_MAX_SRAM_SIZE];
    ...
    memset(SRAM1_ICC, 0, (sizeof(SRAM1_ICC)/sizeof(MS_U16)));
    memset(SRAM2_ICC, 0, (sizeof(SRAM2_ICC)/sizeof(MS_U16)));
    memset(SRAM3_ICC, 0, (sizeof(SRAM3_ICC)/sizeof(MS_U16)));
    memset(SRAM4_ICC, 0, (sizeof(SRAM4_ICC)/sizeof(MS_U16)));

    SC_BK_STORE_NOMUTEX;

      for (u16Index = 0 ;u16Index < SRAM_ICC_TOTAL_COUNT ; u16Index++)
    {
        if (u16Index < SRAM2_ICC_OFFSET)
        {
            u16Ramcode_L = *((MS_U8 *)(u32Addr + (2*u16Index)));
            u16Ramcode_H = *((MS_U8 *)(u32Addr + (2*u16Index+1)));
            SRAM1_ICC[cont1] = ((u16Ramcode_H & 0xFF)<<8)+u16Ramcode_L;
            //printf("@@SRAM1_IHC[%u]=%x\n",cont1,SRAM1_ICC[cont1]);
            cont1++;
        }
        else if (u16Index < SRAM3_ICC_OFFSET)
        {
            u16Ramcode_L = *((MS_U8 *)(u32Addr + (2*u16Index)));
            u16Ramcode_H = *((MS_U8 *)(u32Addr + (2*u16Index+1)));
            SRAM2_ICC[cont2] = ((u16Ramcode_H & 0xFF)<<8)+u16Ramcode_L;
            //printf("@@SRAM2_IHC[%u]=%x\n",cont2,SRAM2_ICC[cont2]);
            cont2++;
        ...
        else if (u16Index < SRAM4_ICC_OFFSET)
        {
            u16Ramcode_L = *((MS_U8 *)(u32Addr + (2*u16Index)));
            u16Ramcode_H = *((MS_U8 *)(u32Addr + (2*u16Index+1)));
            SRAM3_ICC[cont3] = ((u16Ramcode_H & 0xFF)<<8)+u16Ramcode_L;
            //printf("@@SRAM3_IHC[%u]=%x\n",cont3,SRAM3_ICC[cont3]);
            cont3++;
        }
        else
        {
            u16Ramcode_L = *((MS_U8 *)(u32Addr + (2*u16Index)));
            u16Ramcode_H = *((MS_U8 *)(u32Addr + (2*u16Index+1)));
            SRAM4_ICC[cont4] = ((u16Ramcode_H & 0xFF)<<8)+u16Ramcode_L;
            //printf("@@SRAM4_IHC[%u]=%x\n",cont4,SRAM4_ICC[cont4]);
            cont4++;
        }
    }

    _Hal_PQ_set_sram_icc_crd_table(&SRAM1_ICC[0], 0, SRAM1_ICC_COUNT);
    _Hal_PQ_set_sram_icc_crd_table(&SRAM2_ICC[0], 1, SRAM2_ICC_COUNT);
    _Hal_PQ_set_sram_icc_crd_table(&SRAM3_ICC[0], 2, SRAM3_ICC_COUNT);
    _Hal_PQ_set_sram_icc_crd_table(&SRAM4_ICC[0], 3, SRAM4_ICC_COUNT);

    SC_BK_RESTORE_NOMUTEX;
}

void Hal_PQ_get_sram_icc_crd_table(MS_U8 u8SRAM_Idx, MS_U8 *pBuf)
{
    MS_U16 i;
    MS_U16 u16Size = SRAM1_ICC_COUNT;
...
MS_U16 Hal_PQ_get_madi_idx(MS_BOOL bMainWin, MS_MADI_TYPE etype)
{
    ...
    case MS_MADI_25_6F_4R_MC:
        u16ret = PQ_IP_MADi_25_6F_4R_MC_Main;
    break;
...
void Hal_PQ_set_sram_xvycc_gamma_table(MS_U8 u8sramtype, void *pData)
{
    ...
    if (u8sramtype == SC_FILTER_SRAM_XVYCC_DE_GAMMA)
    {
        ...
        for( i = 0; i < 1200; i += 4 )
        {
            u16InputData1 = u8Addr[i]   + (MS_U16)u8Addr[i+1]*256;
            u16InputData2 = u8Addr[i+2] + (MS_U16)u8Addr[i+3]*256;
            u16InputData3 = u8Addr[i+4] + (MS_U16)u8Addr[i+5]*256;

            u32Ref = u16InputData1&0x7FFF; // 15bis
            u32Delta1 = (u16InputData2 - u16InputData1)&0x1F; // 5 bits
            u32Delta2 = (u16InputData3 - u16InputData2)&0x1F; // 5 bits
