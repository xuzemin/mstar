...
static BOOLEAN msKeypad_CH_GetKey(U8 Channel, U8 *pkey, U8* pflag)
{
    ...
    *pflag = 0;
    ...
    for ( i = 0; i < (KEYPAD_ISR_STABLE_NUM); i++ )
    {
        ...
        {
        #if (  (MS_BOARD_TYPE_SEL == BD_MST165A_D01A_S)    \
            || (MS_BOARD_TYPE_SEL == BD_MST165A_D01B_S)    \
            || (MS_BOARD_TYPE_SEL == BD_MST165B_D01A_S)    \
            || (MS_BOARD_TYPE_SEL == BD_MST202A_D01A_S)    \
            || (MS_BOARD_TYPE_SEL == BD_MST043B_10ALB_12351)    \
            || (MS_BOARD_TYPE_SEL==BD_MST043B_10BLB_12413)\
            || (MS_BOARD_TYPE_SEL==BD_MST042B_10BLU_12423)\
            || (MS_BOARD_TYPE_SEL==BD_MST042B_10BLU_12423_32)\
            || (MS_BOARD_TYPE_SEL==BD_MST044B_10BLA_12423)\
            || (MS_BOARD_TYPE_SEL==BD_MST044B_10BLA_12423_32)\
            || (MS_BOARD_TYPE_SEL == BD_MST165C_D01A_S)||(MS_BOARD_TYPE_SEL == BD_MST165C_D01B_S)||(MS_BOARD_TYPE_SEL == BD_MST197A_D01A_S))
...
static BOOLEAN msKeypad_CH_GetKey(U8 Channel, U8 *pkey, U8* pflag)
{
    ...
    if (u8PwrKeypadIntFlag)
   {
    ...
    for ( i = 0; i < KEYPAD_STABLE_NUM; i++ )
    {
        ...
        #endif
            {
                //if (Key_Value != 0xFF)  printf("\e[31;1m Key_Value: %02X \33[m \n", Key_Value);
                KEY_LV[j]++;
                break;
            }
        }
    ...
    for(i=0; i<u8CHLVLs[Channel]; i++)
    {
        if(KEY_LV[i] >= KEYPAD_STABLE_NUM_MIN)
        {
            ...
            *pkey = *(Keymapping+i);
            printf("\033[1;33m CH[%d] = %02X \33[m\n", Channel, *pkey);
            ...
            {
                if(KeyPadCheckCount>KEYPAD_KEY_VALIDATION) // for time-out
                {
                    if (g100msTimeCount > (KeyPadTimePeriod + KEYPAD_REPEAT_PERIOD))
                    {
                        KeyPadCheckCount =0;
                        KeyPadTimePeriod = g100msTimeCount;
                        return MSRET_ERROR;
