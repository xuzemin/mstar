...
{
	E_UART_PARITY_NONE = 0, //do nothing
	E_UART_PARITY_EVEN, //enable uart parity & set EVEN parity
	E_UART_PARITY_ODD, //enable uart parity & set ODD parity
	E_UART_PARITY_DISABLE //close uart parity
} UART_PARITY;


typedef enum _UART_STOP
{
	E_UART_STOP_NONE = 0, //do nothing
	E_UART_STOP_1_BIT, // 1 stop bit
	E_UART_STOP_1_5_BIT, //1.5 stop bit
	E_UART_STOP_2_BIT  // 2 stop bit
} UART_STOP;

typedef enum _UART_CHAR_NUM
{
	E_UART_CHAR_NONE = 0, //do nothing
	E_UART_CHAR_5_BIT, //5 bits in each character
	E_UART_CHAR_6_BIT, //6 bits in each character
	E_UART_CHAR_7_BIT, //7 bits in each character
	E_UART_CHAR_8_BIT //8 bits in each character
} UART_CHAR_NUM;
...
MS_BOOL mdrv_uart_set_config(MS_U32 hUart, UART_Config *config);
