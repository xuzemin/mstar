...
void devTunerSetPLLData(WORD wPLLData, RFBAND eBand)
{
	...
	{
		eVideoSystem = msAPI_ATV_GetVideoStandardOfProgram(msAPI_ATV_GetCurrentProgramNumber());
		if( eVideoSystem!=E_VIDEOSTANDARD_NTSC_M && eVideoSystem!=E_VIDEOSTANDARD_NTSC_44)
		{
			if(eVideoSystem==E_VIDEOSTANDARD_AUTO)
			{
				if (E_AUDIOSTANDARD_M != msAPI_ATV_GetAudioStandard(msAPI_ATV_GetCurrentProgramNumber())) // NTSC M
				{
					PLL_P2=0x04;
				}
			}
    ...
    if(eBand == E_RFBAND_VHF_LOW)
        cTunerData[3] = (0x01+PLL_P2);//P2 =1 PAL+SECAM P2=0 NTSC
    else if (eBand == E_RFBAND_VHF_HIGH)
        cTunerData[3] = (0x02+PLL_P2);
    else
        cTunerData[3] = (0x08+PLL_P2);


    devCOFDM_PassThroughI2C_WriteBytes(_TUNER_I2C_ID, 0, NULL, 4, cTunerData);
    //printf("Tuner Data: %bx, %bx, %bx, %bx\n", cTunerData[0], cTunerData[1], cTunerData[2], cTunerData[3]);
}
