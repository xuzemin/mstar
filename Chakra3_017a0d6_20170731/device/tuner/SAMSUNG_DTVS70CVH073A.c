...
UInt32 devTunerGetStandardType(RF_CHANNEL_BANDWIDTH eBandWidth)
{
	UInt32 dwStandard;
#if ENABLE_T_C_COMBO
...
void devDigitalTuner_Init()
{
   //* Begin
    ...
    TuneRequest.dwStandard = devTunerGetStandardType(E_RF_CH_BAND_6MHz);
    err = tmsysOM3912SendRequest(sysUnit,&TuneRequest,sizeof(TuneRequest), TRT_TUNER_ONLY );
    return ;
}
...
{    
    ...
    TuneRequest.dwStandard = devTunerGetStandardType(eBandWidth);
    tmsysOM3912SendRequest(sysUnit,&TuneRequest,sizeof(TuneRequest), TRT_TUNER_ONLY );
}
