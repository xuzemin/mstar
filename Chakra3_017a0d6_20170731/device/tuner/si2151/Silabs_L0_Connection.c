...
#define  SIMULATOR_SCOPE 0xff//0xffff
...
unsigned char  ucBuffer[10];//ucBuffer[1000];
...
#ifdef    SiTRACES
#define   SiTRACES_BUFFER_LENGTH  10//100000
...
int     L0_WriteRawBytes     (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pbtDataBuffer) {
  ...
  pbtBuffer = (unsigned char*) malloc((iNbBytes + i2c->indexSize)*sizeof(unsigned char));
    ...
    memset(pbtBuffer, 0, (iNbBytes + i2c->indexSize)*sizeof(unsigned char)); // Fix coverity-169081
...
int     L0_WriteString       (L0_Context* i2c, char *writeString) {
 ...
 //   unsigned int hexval;
    ...
    memset(pbtDataBuffer, 0, sizeof(unsigned char)*iNbBytes); // Fix coverity-169082
