...
#include "Si2190B/Si21XXC_L1_API.h"

/* Si2190 API Specific Includes */
#include "Si2190B/Si21XXC_L2_API.h"               /* Include file for this code */
#define Si21XXC_BYTES_PER_LINE 8
#ifdef USING_ALIF_FILTER
#include "Si2190B/write_ALIF_video_coeffs.h"   /* .h file from custom Video filter Tool output */
#endif
#ifdef USING_DLIF_FILTER
#include "Si2190B/write_DLIF_video_coeffs.h"   /* .h file from custom Video filter Tool output */
#endif

static BOOLEAN g_bIsBooted=FALSE; //after boot up, set to TRUE

// static int Si21XXC_printf(const char *format, ...);
...
#include "Si2190B/Si21XXC_User_Properties.c"
#include "Si2190B/Si21XXC_L2_API.c"
#include "Si2190B/Si21XXC_Properties_Strings.c"
#include "Si2190B/Si21XXC_L1_Properties.c"
#include "Si2190B/Si21XXC_L1_API.c"
#include "Si2190B/Si21XXC_L1_Commands.c"
#include "Si2190B/Silabs_L0_Connection.c"
// #include "si2190/silabs_L0_TV_Chassis.cpp"
// #include "si2190/Si21XXC_i2c_api.cpp"

extern IF_FREQ m_eIF_Freq;

#define DBG_TUNER(x)  // x

//PLL lock check parameters
// #define C_Si21XXC_POLL_INTERVAL      5 //ms
// #define C_Si21XXC_PLL_POLL_TIMETOUT      100 //ms
// #define C_Si21XXC_PLL_POLL_CNT           C_Si21XXC_PLL_POLL_TIMETOUT/C_Si21XXC_POLL_INTERVAL //counter
#define C_Si21XXC_PF_TO_CF_SHIFT_B 2250 /*KHZ*/
#define C_Si21XXC_PF_TO_CF_SHIFT_G 2750 /*KHZ*/
#define C_Si21XXC_PF_TO_CF_SHIFT_DK 2750 /*KHZ*/
#define C_Si21XXC_PF_TO_CF_SHIFT_I  2750 /*KHZ*/
#define C_Si21XXC_PF_TO_CF_SHIFT_L  2750 /*KHZ*/
#define C_Si21XXC_PF_TO_CF_SHIFT_M  1750 /*KHZ*/
#define C_Si21XXC_PF_TO_CF_SHIFT_L1 2750 /*KHZ*/

#if 0
// center frequencies
#define C_Si21XXC_IF_CENTER_B  4750 /*KHZ*/
#define C_Si21XXC_IF_CENTER_GH 4250 /*KHZ*/
#define C_Si21XXC_IF_CENTER_DK 5250 /*KHZ*/
#define C_Si21XXC_IF_CENTER_I  4750 /*KHZ*/
#define C_Si21XXC_IF_CENTER_L  5250 /*KHZ*/
#define C_Si21XXC_IF_CENTER_L1 5250 /*KHZ*/
#define C_Si21XXC_IF_CENTER_M  4250 /*KHZ*/
#define C_Si21XXC_IF_CENTER_N  4250 /*KHZ*/
#endif

#define C_Si21XXC_LO_ADDRESS           ((U8)  0xC0)
#define C_Si21XXC_LO_DIVIDER_STEP       ((U32) 1000)  /* Hz */
#define C_Si21XXC_LO_DIVIDER_STEP_ANA   ((U16)  1000)  /* Hz */
#define C_SILABS_Si21XXC_STR              "SILABS_Si2190"

#define SILAB_2190_I2C_ADDR (((U16)TUNER_IIC_BUS<<8)| C_Si21XXC_LO_ADDRESS)
...
// static int Si21XXC_LoadFirmware(L1_Si21XXC_Context *api, unsigned char fw_table[], int nbLines);
// static int Si21XXC_Configure(L1_Si21XXC_Context *api);
static void SetIfDemod(RF_CHANNEL_BANDWIDTH eBandWidth, Network_Type eNetworktype, SI_AnalogTVSystem TV_SYS);
//static U32 Si21XXC_GetDefaultIF(U32 Freq, SI_AnalogTVSystem TV_SYS);
static BOOLEAN Si21XXC_GetRSSILevel(float *strength_dBm);
...
static SI_AnalogTVSystem Atv_system_enum_conversion(IF_FREQ eIF_Freq)
{
...
{
    static L1_Si21XXC_Context _L1_api;
   ...
   //ADD YOU TUNER DRIVER CODE IN HERE
    ...
    api = &_L1_api; //(L1_Si21XXC_Context *)malloc(sizeof(L1_Si21XXC_Context));
    ...
    /* Software Init */
    Si21XXC_L1_API_Init(api,C_Si21XXC_LO_ADDRESS);

    if (Si21XXC_Init(api) != 0 ) {
      printf("\n");
      printf("ERROR ----------------------------------\n");
      // printf("ERROR initializing the Si2190 at 0x%02x!\n",Si21XXC_BASE_ADDRESS);
...
BOOLEAN ATV_SetTune( U32 u32FreqKHz)
{
    static U32 _u32PreFreqKHz = Si21XXC_TUNER_TUNE_FREQ_CMD_FREQ_MIN;
    BOOLEAN retb = FALSE;
    U32 freqHz = 0;
    int count=0;
    U32 start_time = 0;
    ...
    DBG_TUNER(printf("%s, %d, Freq=%ld Khz\n",__FUNCTION__,__LINE__,u32FreqKHz);)

    if(u32FreqKHz)
    {
        _u32PreFreqKHz = u32FreqKHz;
    }
    else
    {
        u32FreqKHz = _u32PreFreqKHz;
    }
    ...
    TV_SYS = Atv_system_enum_conversion(m_eIF_Freq);
#else
    previous_video_sys = api->prop->atv_video_mode.video_sys = Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_M;
    TV_SYS = Atv_system_enum_conversion(IF_FREQ_MN);
#endif
    ...
    DBG_TUNER(printf ("ATV_SetTune freq = %ld; SYS = %d...........\n", u32FreqKHz, TV_SYS);)
        ...
        switch (TV_SYS)
        {
            case si_tv_system_bg:
                ...
                {
                    freqHz = (u32FreqKHz * 1000) + C_Si21XXC_PF_TO_CF_SHIFT_B*1000;
                }
                else
                {
                    freqHz = (u32FreqKHz * 1000) + C_Si21XXC_PF_TO_CF_SHIFT_G*1000;
                }
                api->prop->tuner_lo_injection.band_1      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_1_HIGH_SIDE;
                api->prop->tuner_lo_injection.band_2      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_2_LOW_SIDE;
                api->prop->tuner_lo_injection.band_3      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_3_LOW_SIDE;
                if (Si21XXC_L1_SetProperty2(api,Si21XXC_TUNER_LO_INJECTION_PROP) != 0)
                {
                        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_TUNER_LO_INJECTION_PROP\n"));
                }
                if (u32FreqKHz < 300000 && previous_video_sys != Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_B )
                {
                    api->prop->atv_video_mode.video_sys=Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_B;
                    SetIfDemod(E_RF_CH_BAND_8MHz, E_Network_Type_Analog_T, TV_SYS);
                    break;
                }
                if (u32FreqKHz >= 300000 && previous_video_sys != Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_GH)
                {
                    api->prop->atv_video_mode.video_sys=Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_GH;
                    SetIfDemod(E_RF_CH_BAND_8MHz, E_Network_Type_Analog_T, TV_SYS);
                    break;
                }
                break;
            case si_tv_system_dk:
                freqHz = (u32FreqKHz * 1000) + C_Si21XXC_PF_TO_CF_SHIFT_DK*1000;
                api->prop->tuner_lo_injection.band_1      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_1_HIGH_SIDE;
                api->prop->tuner_lo_injection.band_2      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_2_HIGH_SIDE;
                api->prop->tuner_lo_injection.band_3      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_3_HIGH_SIDE;
                if (Si21XXC_L1_SetProperty2(api,Si21XXC_TUNER_LO_INJECTION_PROP) != 0)
                {
                        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_TUNER_LO_INJECTION_PROP\n"));
                }
                if (previous_video_sys != Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_DK)
                {
                    SetIfDemod(E_RF_CH_BAND_8MHz, E_Network_Type_Analog_T, TV_SYS);
                }
                break;
            case si_tv_system_i:
                freqHz = (u32FreqKHz * 1000) + C_Si21XXC_PF_TO_CF_SHIFT_I*1000;
                api->prop->tuner_lo_injection.band_1      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_1_HIGH_SIDE;
                api->prop->tuner_lo_injection.band_2      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_2_HIGH_SIDE;
                api->prop->tuner_lo_injection.band_3      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_3_HIGH_SIDE;
                if (Si21XXC_L1_SetProperty2(api,Si21XXC_TUNER_LO_INJECTION_PROP) != 0)
                ...
                if (previous_video_sys != Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_I)
                {
                    SetIfDemod(E_RF_CH_BAND_8MHz, E_Network_Type_Analog_T, TV_SYS);
                }
                break;
            case si_tv_system_m:
                freqHz = (u32FreqKHz * 1000) + C_Si21XXC_PF_TO_CF_SHIFT_M*1000;
                api->prop->tuner_lo_injection.band_1      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_1_HIGH_SIDE;
                api->prop->tuner_lo_injection.band_2      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_2_HIGH_SIDE;
                api->prop->tuner_lo_injection.band_3      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_3_HIGH_SIDE;
                if (Si21XXC_L1_SetProperty2(api,Si21XXC_TUNER_LO_INJECTION_PROP) != 0)
                ...
                if ((previous_video_sys != Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_M) || (!g_bIsBooted))
                {
                    g_bIsBooted = TRUE;
                    SetIfDemod(E_RF_CH_BAND_8MHz, E_Network_Type_Analog_T, TV_SYS);
                }
                break;
            case si_tv_system_l:
                freqHz = (u32FreqKHz * 1000) + C_Si21XXC_PF_TO_CF_SHIFT_L*1000;
                api->prop->tuner_lo_injection.band_1      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_1_HIGH_SIDE;
                api->prop->tuner_lo_injection.band_2      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_2_LOW_SIDE;
                api->prop->tuner_lo_injection.band_3      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_3_LOW_SIDE;
                if (Si21XXC_L1_SetProperty2(api,Si21XXC_TUNER_LO_INJECTION_PROP) != 0)
                ...
                if (previous_video_sys != Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_L)
                {
                    SetIfDemod(E_RF_CH_BAND_8MHz, E_Network_Type_Analog_T, TV_SYS);
                }
                break;
            case si_tv_system_lp:
                freqHz = (u32FreqKHz * 1000) - C_Si21XXC_PF_TO_CF_SHIFT_L1*1000;
                api->prop->tuner_lo_injection.band_1      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_1_HIGH_SIDE;
                api->prop->tuner_lo_injection.band_2      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_2_LOW_SIDE;
                api->prop->tuner_lo_injection.band_3      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_3_LOW_SIDE;
                if (Si21XXC_L1_SetProperty2(api,Si21XXC_TUNER_LO_INJECTION_PROP) != 0)
                ...
                if (previous_video_sys != Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_LP)
                {
                    SetIfDemod(E_RF_CH_BAND_8MHz, E_Network_Type_Analog_T, TV_SYS);
                }
                break;
        ...
        if (freqHz < Si21XXC_TUNER_TUNE_FREQ_CMD_FREQ_MIN || freqHz > Si21XXC_TUNER_TUNE_FREQ_CMD_FREQ_MAX)
        {
        ...
        if (Si21XXC_L1_TUNER_TUNE_FREQ(api,Si21XXC_TUNER_TUNE_FREQ_CMD_MODE_ATV,  freqHz) != 0)
        {
            printf("[Silabs]:Error Si21XXC_L1_TUNER_TUNE_FREQ\n");
            mcSHOW_HW_MSG((" [Silabs]: Error Si21XXC_L1_TUNER_TUNE_FREQ\n"));
        }
    ...
    if(tuner_init)
    {
        ...
        {
            if (Si21XXC_L1_CheckStatus(api) != 0)
                return FALSE;
...
BOOLEAN DTV_SetTune(double Freq, RF_CHANNEL_BANDWIDTH eBandWidth, EN_TUNER_MODE eMode)
{
    ...
    U32 start_time = 0;
    U32 timeout = 0;
    Network_Type eNetworktype = E_Network_Type_ATSC;
    ...
    DBG_TUNER(printf ("Si21XXC_TunerSetFreq freq = %d; Band = %d Network = %d\n", (U16)Freq,eBandWidth, eNetworktype);)
    ...
    if(tuner_init)
    {
        ...
        switch (eNetworktype)
        {
           case E_Network_Type_DVBT:
                if ( (previous_dtv_mode != Si21XXC_DTV_MODE_PROP_MODULATION_DVBT)
                    || (previous_agc_speed!=Si21XXC_DTV_AGC_SPEED_PROP_IF_AGC_SPEED_AUTO)
                    || (eBandWidth != pre_bw)
                    )
           ...
           case E_Network_Type_DVBT2:
                if ( (previous_dtv_mode != Si21XXC_DTV_MODE_PROP_MODULATION_DVBT)
                    || (previous_agc_speed!=Si21XXC_DTV_AGC_SPEED_PROP_IF_AGC_SPEED_39)
                    || (eBandWidth != pre_bw)
                   )
                    SetIfDemod(eBandWidth, eNetworktype, si_tv_system_invalid);
                break;
           case E_Network_Type_DVBC:
                if (previous_dtv_mode != Si21XXC_DTV_MODE_PROP_MODULATION_DVBC)
                    SetIfDemod(eBandWidth, eNetworktype, si_tv_system_invalid);
                break;
           case E_Network_Type_ISDBT:
                if (previous_dtv_mode != Si21XXC_DTV_MODE_PROP_MODULATION_ISDBT)
                    SetIfDemod(eBandWidth, eNetworktype, si_tv_system_invalid);
                break;
           case E_Network_Type_ISDBC:
                if (previous_dtv_mode != Si21XXC_DTV_MODE_PROP_MODULATION_ISDBC)
                    SetIfDemod(eBandWidth, eNetworktype, si_tv_system_invalid);
                break;
           case E_Network_Type_DTMB:
                if (previous_dtv_mode != Si21XXC_DTV_MODE_PROP_MODULATION_DTMB)
                    SetIfDemod(eBandWidth, eNetworktype, si_tv_system_invalid);
                break;
           case E_Network_Type_ATSC:
                if (previous_dtv_mode != Si21XXC_DTV_MODE_PROP_MODULATION_ATSC)
                    SetIfDemod(eBandWidth, eNetworktype, si_tv_system_invalid);
                break;
           case E_Network_Type_QAM_US:
                if (previous_dtv_mode != Si21XXC_DTV_MODE_PROP_MODULATION_QAM_US)
                    SetIfDemod(eBandWidth, eNetworktype, si_tv_system_invalid);
                break;
    ...
    printf("%s, %d, Freq=%d Hz, bw=%d, mode=%d\n",__FUNCTION__,__LINE__,freqHz,eBandWidth,eMode);
        ...
        if (Si21XXC_L1_TUNER_TUNE_FREQ(api,Si21XXC_TUNER_TUNE_FREQ_CMD_MODE_DTV,  freqHz) != 0)
        {
            printf("[Silabs]:Error Si21XXC_L1_TUNER_TUNE_FREQ\n");
            mcSHOW_HW_MSG((" [Silabs]: Error Si21XXC_L1_TUNER_TUNE_FREQ\n"));
            return FALSE;
...
BOOLEAN ExtendCommand(U8 u8SubCmd, U32 u32Param1, U32 u32Param2, void* pvoidParam3)
{
    ...
    switch(u8SubCmd)
    {
        ...
        case mapi_tuner_datatype::E_TUNER_SUBCMD_GET_IF_FREQ:
        {
            ...
            *u16IFFreq = C_Si21XXC_IF_CENTER_B;
        }
            break;
        ...
        {
            U16 *u16IFFreq = (U16 *)pvoidParam3;
            *u16IFFreq = C_Si21XXC_IF_CENTER_L1;
        }
            break;
                ...
                case E_TUNER_DTV_DVB_T2_MODE:
                    switch(u32Param2) // bandwidth
                    {
                        case E_RF_CH_BAND_6MHz:
                            *((U32 *)pvoidParam3)=C_Si21XXC_IF_FREQUENCY;
                            break;
        ...
        case mapi_tuner_datatype::E_TUNER_SUBCMD_VIF_INIT:
        {
            ...
            api->prop->atv_afc_range.range_khz = Si21XXC_ATV_AFC_RANGE_PROP_RANGE_KHZ_DEFAULT;
            if (Si21XXC_L1_SetProperty2(api, Si21XXC_ATV_AFC_RANGE_PROP_CODE               ) != NO_Si21XXC_ERROR)
            {
        ...
        case mapi_tuner_datatype::E_TUNER_SUBCMD_VIF_SET_SOUND_SYSTEM:
        {
            switch (u32Param1)
            {
                ...
                case mapi_vif_datatype::E_VIF_SOUND_GH_MONO_NICAM:
                    ...
                    break;
                case mapi_vif_datatype::E_VIF_SOUND_I:
                    api->prop->atv_video_mode.video_sys = Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_I;
                    break;
                case mapi_vif_datatype::E_VIF_SOUND_DK1_STEREO_A2:
                case mapi_vif_datatype::E_VIF_SOUND_DK2_STEREO_A2:
                case mapi_vif_datatype::E_VIF_SOUND_DK3_STEREO_A2:
                case mapi_vif_datatype::E_VIF_SOUND_DK_MONO_NICAM:
                    api->prop->atv_video_mode.video_sys = Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_DK;
                    break;
                case mapi_vif_datatype::E_VIF_SOUND_L:
                    api->prop->atv_video_mode.video_sys = Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_L;
                    break;
                case mapi_vif_datatype::E_VIF_SOUND_LL:
                    api->prop->atv_video_mode.video_sys = Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_LP;
                    break;
                case mapi_vif_datatype::E_VIF_SOUND_MN:
                    api->prop->atv_video_mode.video_sys = Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_M;
                    break;
            }
            if (Si21XXC_L1_SetProperty2(api, Si21XXC_ATV_VIDEO_MODE_PROP_CODE) != NO_Si21XXC_ERROR)
            {
                printf("Error E_TUNER_SUBCMD_VIF_SET_AFC_RANGE\n");
                return FALSE;
            }
        ...
        {
            if (Si21XXC_L1_ATV_STATUS(api, Si21XXC_ATV_STATUS_CMD_INTACK_CLEAR) != NO_Si21XXC_ERROR) { /* execute ATV_STATUS and always read response */
        ...
        case mapi_tuner_datatype::E_TUNER_SUBCMD_VIF_GET_FOE:
        {
           ...
           //Si21XXC_L1_GetCommandResponseString(Si2190, Si21XXC_ATV_STATUS_CMD_CODE, "\n", message);
        ...
        {
            unsigned char gpio2_mode = Si21XXC_CONFIG_PINS_CMD_GPIO2_MODE_NO_CHANGE;
            if (tuner_init == FALSE)
            {
                printf("[Error]%s,%d\n",__FILE__,__LINE__);
                return FALSE;
            ...
            gpio2_mode = (u32Param1 == TRUE) ? Si21XXC_CONFIG_PINS_CMD_GPIO2_MODE_DRIVE_1 : Si21XXC_CONFIG_PINS_CMD_GPIO2_MODE_DRIVE_0;
            if (Si21XXC_L1_CONFIG_PINS(api, Si21XXC_CONFIG_PINS_CMD_GPIO1_MODE_NO_CHANGE, Si21XXC_CONFIG_PINS_CMD_GPIO1_READ_DO_NOT_READ,
                                                                  gpio2_mode, Si21XXC_CONFIG_PINS_CMD_GPIO2_READ_DO_NOT_READ,
                                                                  0, 0, 0) != NO_Si21XXC_ERROR) {
                printf("Error Si21XXC_L1_CONFIG_PINS %ld\n", u32Param1);
                return FALSE;
            }
...
void devTunerInit(void)
{
    ...
    if (bRet == FALSE)
    {
     ...
     {
         ATV_SetTune(Si21XXC_TUNER_TUNE_FREQ_CMD_FREQ_MIN + FREQ_STEP*1000);
         tuner_init = TRUE;
         g_stFactorySetting.eTunerType = TUNER_SI2190B;
     }
...
/************************************************************************************************************************
  ...
  DESCRIPTON: Load firmware from FIRMWARE_TABLE array in Si21XXC_Firmware_x_y_build_z.h file into Si2158
...
************************************************************************************************************************/
static int Si21XXC_LoadFirmware(L1_Si21XXC_Context *p_api, unsigned char fw_table[], int nbLines)
{
    int return_code = 0;
    int line;
    DBG_TUNER(printf("Si21XXC_LoadFirmware nbLines %d Begin.......\n", nbLines);)
    /* for each 8 bytes in fw_table */
    for (line = 0; line < nbLines; line++)
    {
        /* send that 8 byte I2C command to Si2158 */
        if ((return_code = Si21XXC_L1_API_Patch(p_api, 8, fw_table+8*line)) != 0)
        {
          printf("Si21XXC_LoadFirmware error 0x%02x patching line %d: %s\n", return_code, line, Si21XXC_L1_API_ERROR_TEXT(return_code) );
          if (line == 0) {
    ...
    DBG_TUNER(printf("Si21XXC_LoadFirmware complete...\n");)
    return 0;
}
...
************************************************************************************************************************/
static int Si21XXC_Configure           (L1_Si21XXC_Context *p_api)
{
    int return_code;
    return_code=0;

    /* Set All Properties startup configuration */
    p_api->prop->tuner_ien.tcien = Si21XXC_TUNER_IEN_PROP_TCIEN_ENABLE;
    p_api->prop->tuner_ien.rssilien = Si21XXC_TUNER_IEN_PROP_RSSILIEN_DISABLE;
    p_api->prop->tuner_ien.rssihien = Si21XXC_TUNER_IEN_PROP_RSSIHIEN_DISABLE;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api,Si21XXC_TUNER_IEN_PROP)) != 0)
    {
       mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_TUNER_IEN_PROP\n"));
       return return_code;
    }
    p_api->prop->dtv_ien.chlien = Si21XXC_DTV_IEN_PROP_CHLIEN_ENABLE;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api,Si21XXC_DTV_IEN_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_IEN_PROP\n"));
       return return_code;
    }
     p_api->prop->atv_ien.chlien = Si21XXC_ATV_IEN_PROP_CHLIEN_ENABLE;
     p_api->prop->atv_ien.pclien = Si21XXC_ATV_IEN_PROP_PCLIEN_DISABLE;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api,Si21XXC_ATV_IEN_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_ATV_IEN_PROP\n"));
       return return_code;
    }
    p_api->prop->dtv_mode.bw = Si21XXC_DTV_MODE_PROP_BW_BW_8MHZ;
    p_api->prop->dtv_mode.modulation = Si21XXC_DTV_MODE_PROP_MODULATION_DVBT;
    p_api->prop->dtv_mode.invert_spectrum = Si21XXC_DTV_MODE_PROP_INVERT_SPECTRUM_INVERTED;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api,Si21XXC_DTV_MODE_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_MODE_PROP\n"));
       return return_code;
    }

    p_api->prop->atv_video_mode.color= Si21XXC_ATV_VIDEO_MODE_PROP_COLOR_PAL_NTSC;
    p_api->prop->atv_video_mode.invert_spectrum= Si21XXC_ATV_VIDEO_MODE_PROP_INVERT_SPECTRUM_INVERTED;
    p_api->prop->atv_video_mode.video_sys = Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_GH;
   if ((return_code=Si21XXC_L1_SetProperty2(p_api,Si21XXC_ATV_VIDEO_MODE_PROP)) != 0)
    {
       mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_ATV_VIDEO_MODE_PROP\n"));
       return return_code;
    }


    p_api->prop->atv_config_if_port.atv_agc_source = Si21XXC_ATV_CONFIG_IF_PORT_PROP_ATV_AGC_SOURCE_AGC2_3DB;
    p_api->prop->atv_config_if_port.atv_out_type = Si21XXC_ATV_CONFIG_IF_PORT_PROP_ATV_OUT_TYPE_LIF_DIFF_IF1;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api, Si21XXC_ATV_CONFIG_IF_PORT_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_ATV_CONFIG_IF_PORT_PROP\n"));
        return return_code;
    }

   p_api->prop->atv_lif_freq.offset = C_Si21XXC_IF_CENTER_GH;
   if ((return_code=Si21XXC_L1_SetProperty2(p_api, Si21XXC_ATV_LIF_FREQ_PROP)) != 0)
   {
       mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_ATV_LIF_FREQ_PROP\n"));
       return return_code;
   }
   p_api->prop->tuner_lo_injection.band_1      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_1_HIGH_SIDE;
   p_api->prop->tuner_lo_injection.band_2      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_2_HIGH_SIDE;
   p_api->prop->tuner_lo_injection.band_3      = Si21XXC_TUNER_LO_INJECTION_PROP_BAND_3_HIGH_SIDE;
   if ((return_code=Si21XXC_L1_SetProperty2(p_api,Si21XXC_TUNER_LO_INJECTION_PROP)) != 0)
   {
           mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_TUNER_LO_INJECTION_PROP\n"));
           return return_code;
   }
    p_api->prop->atv_agc_speed.if_agc_speed= Si21XXC_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_185;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api,Si21XXC_ATV_AGC_SPEED_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_ATV_AGC_SPEED_PROP\n"));
        return return_code;
    }
    p_api->prop->atv_ext_agc.max_10mv= Si21XXC_ATV_EXT_AGC_PROP_MAX_10MV_DEFAULT;
    p_api->prop->atv_ext_agc.min_10mv= Si21XXC_ATV_EXT_AGC_PROP_MIN_10MV_DEFAULT;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api,Si21XXC_ATV_EXT_AGC_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_ATV_EXT_AGC_PROP\n"));
        return return_code;
    }

//    p_api->prop->atv_afc_range.range_khz= 2000;
    p_api->prop->atv_afc_range.range_khz= 100;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api, Si21XXC_ATV_AFC_RANGE_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_ATV_AFC_RANGE_PROP\n"));
        return return_code;
    }
    p_api->prop->atv_lif_out.offset = Si21XXC_ATV_LIF_OUT_PROP_OFFSET_DEFAULT;
    p_api->prop->atv_lif_out.amp = 148;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api, Si21XXC_ATV_LIF_OUT_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_ATV_LIF_OUT_PROP\n"));
        return return_code;
    }
    p_api->prop->dtv_config_if_port.dtv_out_type   = Si21XXC_DTV_CONFIG_IF_PORT_PROP_DTV_OUT_TYPE_LIF_IF1;
    p_api->prop->dtv_config_if_port.dtv_agc_source = Si21XXC_DTV_CONFIG_IF_PORT_PROP_DTV_AGC_SOURCE_AGC2_3DB;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api,Si21XXC_DTV_CONFIG_IF_PORT_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_CONFIG_IF_PROP\n"));
        return return_code;
    }

    // Si21XXC_DTV_PGA_LIMITS_PROP
    p_api->prop->dtv_pga_limits.max = 56;
    p_api->prop->dtv_pga_limits.min = 24;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api,Si21XXC_DTV_PGA_LIMITS_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_PGA_LIMITS_PROP\n"));
        return return_code;
    }

    // p_api->prop->dtv_rf_top.dtv_rf_top= Si21XXC_DTV_RF_TOP_PROP_DTV_RF_TOP_M2DB;
    p_api->prop->dtv_rf_top.dtv_rf_top= Si21XXC_DTV_RF_TOP_PROP_DTV_RF_TOP_P6DB;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api, Si21XXC_DTV_RF_TOP_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_RF_TOP_PROP\n"));
        return return_code;
    }
    p_api->prop->dtv_filter_select.filter             = Si21XXC_DTV_FILTER_SELECT_PROP_FILTER_CUSTOM1; /* (default 'DEFAULT') */
    if ((return_code=Si21XXC_L1_SetProperty2(p_api,Si21XXC_DTV_FILTER_SELECT_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_FILTER_SELECT_PROP\n"));
        return return_code;
    }
    p_api->prop->dtv_lif_out.offset = Si21XXC_DTV_LIF_OUT_PROP_OFFSET_DEFAULT;
    ...
    if ((return_code=Si21XXC_L1_SetProperty2(p_api, Si21XXC_DTV_LIF_OUT_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_LIF_OUT_PROP\n"));
        return return_code;
    }
    p_api->prop->dtv_lif_freq.offset=C_Si21XXC_IF_FREQUENCY;
    if ((return_code=Si21XXC_L1_SetProperty2(p_api, Si21XXC_DTV_LIF_FREQ_PROP)) != 0)
    {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_LIF_FREQ_PROP\n"));
        return return_code;
    }
...
{
    int return_code = 0;
    if ((return_code=Si21XXC_L1_TUNER_STATUS(api,Si21XXC_TUNER_STATUS_CMD_INTACK_CLEAR))!=0) {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_READING_COMMAND Si21XXC_TUNER_STATUS\n"));
        return FALSE;
    }
...
************************************************************************************************************************/
BOOLEAN Si21XXC_GetATV_AFC_freq(int* AFC_freqHz)
{
    int return_code = 0;

    if ((return_code=Si21XXC_L1_ATV_STATUS(api,Si21XXC_ATV_STATUS_CMD_INTACK_OK))!=0) {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_READING_COMMAND Si21XXC_ATV_STATUS\n"));
        printf("[Error]%s, %d\n",__FILE__,__LINE__);
        return FALSE;
    }
    *AFC_freqHz= api->rsp->atv_status.afc_freq;
...
{
    return Si21XXC_GetRSSILevel(f_strength);
}
...
static void SetIfDemod(RF_CHANNEL_BANDWIDTH eBandWidth, Network_Type eNetworktype, SI_AnalogTVSystem TV_SYS)
{
    ...
    if(TV_SYS == si_tv_system_invalid)
    {
        ...
        {
           case E_Network_Type_DVBT:
                api->prop->dtv_mode.modulation = Si21XXC_DTV_MODE_PROP_MODULATION_DVBT;
                if(eBandWidth == E_RF_CH_BAND_6MHz)
                    api->prop->dtv_mode.bw = Si21XXC_DTV_MODE_PROP_BW_BW_6MHZ;
                else if (eBandWidth == E_RF_CH_BAND_7MHz)
                    api->prop->dtv_mode.bw = Si21XXC_DTV_MODE_PROP_BW_BW_7MHZ;
                else
                    api->prop->dtv_mode.bw = Si21XXC_DTV_MODE_PROP_BW_BW_8MHZ;

                api->prop->dtv_mode.invert_spectrum = Si21XXC_DTV_MODE_PROP_INVERT_SPECTRUM_INVERTED;

                if (Si21XXC_L1_SetProperty2(api, Si21XXC_DTV_MODE_PROP) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_MODE_PROP\n"));
                }
                api->prop->dtv_agc_speed.if_agc_speed = Si21XXC_DTV_AGC_SPEED_PROP_IF_AGC_SPEED_AUTO;
                api->prop->dtv_agc_speed.agc_decim = Si21XXC_DTV_AGC_SPEED_PROP_AGC_DECIM_OFF;
                if (Si21XXC_L1_SetProperty2(api, Si21XXC_DTV_AGC_SPEED_PROP) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_AGC_SPEED_PROP\n"));
                }
                api->prop->dtv_initial_agc_speed_period.period = 0;
                if (Si21XXC_L1_SetProperty2(api, Si21XXC_DTV_INITIAL_AGC_SPEED_PERIOD_PROP) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_INITIAL_AGC_SPEED_PERIOD_PROP\n"));
                }
                /* AMP=17 to get IF_AGC=1.24Vrms in DVB-T when AMP is controlled by demod AGC (DTV_AGC_SOURCE=DLIF_AGC_3DB) */
                api->prop->dtv_lif_out.amp =27; // 20 for T3
                if (Si21XXC_L1_SetProperty2(api, Si21XXC_DTV_LIF_OUT_PROP) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_LIF_OUT_PROP\n"));
                }
                api->prop->dtv_config_if_port.dtv_agc_source    =     Si21XXC_DTV_CONFIG_IF_PORT_PROP_DTV_AGC_SOURCE_AGC1_3DB;
                if (Si21XXC_L1_SetProperty2(api, Si21XXC_DTV_CONFIG_IF_PORT_PROP) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_LIF_OUT_PROP\n"));
                }
                break;
           case E_Network_Type_DVBT2:
                ...
                api->prop->dtv_agc_speed.if_agc_speed = Si21XXC_DTV_AGC_SPEED_PROP_IF_AGC_SPEED_39;
                api->prop->dtv_agc_speed.agc_decim = Si21XXC_DTV_AGC_SPEED_PROP_AGC_DECIM_4;
                if (Si21XXC_L1_SetProperty2(api, Si21XXC_DTV_AGC_SPEED_PROP) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_AGC_SPEED_PROP\n"));
                }
                ...
                break;
           case E_Network_Type_DVBC:
                api->prop->dtv_mode.modulation = Si21XXC_DTV_MODE_PROP_MODULATION_DVBC;
                api->prop->dtv_mode.bw = Si21XXC_DTV_MODE_PROP_BW_BW_8MHZ;
                api->prop->dtv_mode.invert_spectrum = Si21XXC_DTV_MODE_PROP_INVERT_SPECTRUM_DEFAULT;
                ...
                break;
           case E_Network_Type_ISDBT:
                api->prop->dtv_mode.modulation = Si21XXC_DTV_MODE_PROP_MODULATION_ISDBT;
                api->prop->dtv_mode.bw = Si21XXC_DTV_MODE_PROP_BW_BW_6MHZ;
                api->prop->dtv_mode.invert_spectrum = Si21XXC_DTV_MODE_PROP_INVERT_SPECTRUM_INVERTED;
                if (Si21XXC_L1_SetProperty2(api, Si21XXC_DTV_MODE_PROP) != 0)
                {
                ...
                if (Si21XXC_L1_SetProperty2(api, Si21XXC_DTV_LIF_FREQ_PROP) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_LIF_FREQ_PROP\n"));
                }
                api->prop->dtv_config_if_port.dtv_agc_source    =     Si21XXC_DTV_CONFIG_IF_PORT_PROP_DTV_AGC_SOURCE_AGC1_3DB;
                ...
                break;
           case E_Network_Type_ATSC:
                api->prop->dtv_mode.modulation = Si21XXC_DTV_MODE_PROP_MODULATION_ATSC;
                api->prop->dtv_mode.bw = Si21XXC_DTV_MODE_PROP_BW_BW_6MHZ;
                api->prop->dtv_mode.invert_spectrum = Si21XXC_DTV_MODE_PROP_INVERT_SPECTRUM_NORMAL;
                ...
                if (Si21XXC_L1_SetProperty2(api, Si21XXC_DTV_LIF_FREQ_PROP) != 0)
                {
                 ...
                 api->prop->tuner_return_loss.mode = Si21XXC_TUNER_RETURN_LOSS_PROP_MODE_TERRESTRIAL;
                 if (Si21XXC_L1_SetProperty2(api, Si21XXC_TUNER_RETURN_LOSS_PROP_CODE) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_TUNER_RETURN_LOSS_PROP_CODE\n"));
                }
#ifdef Si21XXC_DTV_PGA_LIMITS_PROP
                api->prop->dtv_pga_limits.max = 56;
                api->prop->dtv_pga_limits.min = 24;
                if (Si21XXC_L1_SetProperty2(api,Si21XXC_DTV_PGA_LIMITS_PROP) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_DTV_PGA_LIMITS_PROP\n"));
                }
#endif
                ...
                break;
           case E_Network_Type_QAM_US:
                api->prop->dtv_mode.modulation = Si21XXC_DTV_MODE_PROP_MODULATION_QAM_US;
                api->prop->dtv_mode.bw = Si21XXC_DTV_MODE_PROP_BW_BW_6MHZ;
                api->prop->dtv_mode.invert_spectrum = Si21XXC_DTV_MODE_PROP_INVERT_SPECTRUM_NORMAL;
                ...
                break;
           case E_Network_Type_DTMB:
                api->prop->dtv_mode.modulation = Si21XXC_DTV_MODE_PROP_MODULATION_DTMB;
                api->prop->dtv_mode.bw = Si21XXC_DTV_MODE_PROP_BW_BW_8MHZ;
                api->prop->dtv_mode.invert_spectrum = Si21XXC_DTV_MODE_PROP_INVERT_SPECTRUM_INVERTED;
                if (Si21XXC_L1_SetProperty2(api, Si21XXC_DTV_MODE_PROP) != 0)
                {
    ...
    else
    {
        ...
        {
            case si_tv_system_bg:
                api->prop->atv_audio_mode.audio_sys = Si21XXC_ATV_AUDIO_MODE_PROP_AUDIO_SYS_MONO_NICAM;
                api->prop->atv_audio_mode.chan_bw = Si21XXC_ATV_AUDIO_MODE_PROP_CHAN_BW_8X_OVERMOD;
                api->prop->atv_video_mode.color = Si21XXC_ATV_VIDEO_MODE_PROP_COLOR_PAL_NTSC;
                // api->prop->atv_video_mode.trans = Si21XXC_ATV_VIDEO_MODE_PROP_TRANS_CABLE;
        		api->prop->atv_cvbs_out_fine.amp = 49;
        		api->prop->atv_cvbs_out_fine.offset = 32;
                break;
            case si_tv_system_dk:
                api->prop->atv_audio_mode.audio_sys = Si21XXC_ATV_AUDIO_MODE_PROP_AUDIO_SYS_A2_DK4;
                api->prop->atv_audio_mode.chan_bw = Si21XXC_ATV_AUDIO_MODE_PROP_CHAN_BW_8X_OVERMOD;
                api->prop->atv_video_mode.video_sys = Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_DK;
                api->prop->atv_video_mode.color = Si21XXC_ATV_VIDEO_MODE_PROP_COLOR_PAL_NTSC;
                // api->prop->atv_video_mode.trans = Si21XXC_ATV_VIDEO_MODE_PROP_TRANS_CABLE;
        		api->prop->atv_cvbs_out_fine.amp = 48;
        		api->prop->atv_cvbs_out_fine.offset = 32;
                break;
            case si_tv_system_i:
                api->prop->atv_audio_mode.audio_sys = Si21XXC_ATV_AUDIO_MODE_PROP_AUDIO_SYS_MONO_NICAM;
                ...
                api->prop->atv_video_mode.color = Si21XXC_ATV_VIDEO_MODE_PROP_COLOR_PAL_NTSC;
                // api->prop->atv_video_mode.trans = Si21XXC_ATV_VIDEO_MODE_PROP_TRANS_CABLE;
        		api->prop->atv_cvbs_out_fine.amp = 44;
        		api->prop->atv_cvbs_out_fine.offset = 32;
                break;
            case si_tv_system_m:
                api->prop->atv_audio_mode.audio_sys = Si21XXC_ATV_AUDIO_MODE_PROP_AUDIO_SYS_A2;
                api->prop->atv_audio_mode.chan_bw = Si21XXC_ATV_AUDIO_MODE_PROP_CHAN_BW_DEFAULT;
                api->prop->atv_video_mode.video_sys = Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_M;
                api->prop->atv_video_mode.color = Si21XXC_ATV_VIDEO_MODE_PROP_COLOR_PAL_NTSC;
                // api->prop->atv_video_mode.trans = Si21XXC_ATV_VIDEO_MODE_PROP_TRANS_CABLE;
        		api->prop->atv_cvbs_out_fine.amp = 51;//53;//49;
        		api->prop->atv_cvbs_out_fine.offset = 32;
                break;
            case si_tv_system_l:
                api->prop->atv_audio_mode.audio_sys = Si21XXC_ATV_AUDIO_MODE_PROP_AUDIO_SYS_MONO_NICAM;
                api->prop->atv_audio_mode.chan_bw = Si21XXC_ATV_AUDIO_MODE_PROP_CHAN_BW_8X_OVERMOD;
                api->prop->atv_video_mode.video_sys = Si21XXC_ATV_VIDEO_MODE_PROP_VIDEO_SYS_L;
                api->prop->atv_video_mode.color = Si21XXC_ATV_VIDEO_MODE_PROP_COLOR_SECAM;
                // api->prop->atv_video_mode.trans = Si21XXC_ATV_VIDEO_MODE_PROP_TRANS_CABLE;
        		api->prop->atv_cvbs_out_fine.amp = 53;
        		api->prop->atv_cvbs_out_fine.offset = 32;
                break;
            case si_tv_system_lp:
                api->prop->atv_audio_mode.audio_sys = Si21XXC_ATV_AUDIO_MODE_PROP_AUDIO_SYS_MONO_NICAM;
        ...
        if (Si21XXC_L1_SetProperty2(api, Si21XXC_ATV_VIDEO_MODE_PROP) != 0)
        {
            mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_ATV_VIDEO_MODE_PROP\n"));
        }
        if (Si21XXC_L1_SetProperty2(api, Si21XXC_ATV_CVBS_OUT_FINE_PROP_CODE) != 0)
        {
            mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_ATV_VIDEO_MODE_PROP\n"));
        }
        if(!scan_mode)
        {
            if (Si21XXC_L1_SetProperty2(api, Si21XXC_ATV_AUDIO_MODE_PROP) != 0)
            {
                mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si21XXC_ATV_AUDIO_MODE_PROP\n"));
            }
        }
    }
}
...
static U32 Si21XXC_GetDefaultIF(U32 Freq, SI_AnalogTVSystem TV_SYS)
{
    ...
    switch (TV_SYS)
    {
        case si_tv_system_bg:
            if (Freq < 300000)
            {
                offset = C_Si21XXC_IF_CENTER_B;
                break;
            }
            if (Freq >= 300000)
            {
                offset = C_Si21XXC_IF_CENTER_GH;
                break;
            }
            break;
        case si_tv_system_dk:
            offset = C_Si21XXC_IF_CENTER_DK;
            break;
        case si_tv_system_i:
            offset = C_Si21XXC_IF_CENTER_I;
            break;
        case si_tv_system_m:
            offset = C_Si21XXC_IF_CENTER_M;
            break;
        case si_tv_system_l:
            offset = C_Si21XXC_IF_CENTER_L;
            break;
        case si_tv_system_lp:
            offset = C_Si21XXC_IF_CENTER_L1;
            break;
        case si_tv_system_invalid:
        default:
            printf("[Error]%s, %s, %d\n",__FILE__,__FUNCTION__,__LINE__);
...
#endif
#if 0
int Si21XXC_printf(const char *format, ...)
{
    format = format;
    // printf(format);
    return 0;
...
void devTunerSetFreq_ATSC(U16 u16Freq_over_freq_step_khz, EN_DEMOD_TYPE enModulation)
{
    ...
    DBG_TUNER(printf("[devTunerSetFreq_ATSC]%s, %d,  enModulation=%d, u16Freq_over_freq_step_khz=%d, u32Freq=%ld Hz\n",__FILE__,__LINE__,enModulation,u16Freq_over_freq_step_khz,u32Freq));

    if(enModulation == DEMOD_TYPE_NTSC)
    {
#if FRONTEND_IF_DEMODE_TYPE == SAMSUNG_SI21XX
       retb &= ATV_SetTune(u32Freq/1000);
#else
       retb &= DTV_SetTune((double)u32Freq/1000000.0,E_RF_CH_BAND_8MHz, E_TUNER_ATV_NTSC_MODE);
#endif
        CNTFineTuneStep = 0;
    }
...
void devTuner_SetFineTune(S8 step)
{
	CNTFineTuneStep += 1*step;

/* step : step +1 or -1 of 62.5 KHz, Si21XX offset = 500hz */
    int ifreq = CNTFineTuneStep*125;

    printf("devTuner_SetFineTune %d\n", ifreq);

    if (Si21XXC_L1_FINE_TUNE (api, Si21XXC_FINE_TUNE_CMD_PERSISTENCE_PERSISTENT, ifreq) != NO_Si21XXC_ERROR)
    {
        printf("[Error]%s,%d\n",__FILE__,__LINE__);
        return;
    }
...
S8 devTuner_GetFineTune(void)
{
    return CNTFineTuneStep;
}

S16 devTunerGetFreqOffset(void)
{
	int ifreq;

	if(Si21XXC_GetATV_AFC_freq(&ifreq))
	{
		return ifreq;
	}
	return 0;
...
BYTE DevSi21XXGetTunerSyncLock(void)
{
    int return_code = 0;
	...
	return ((api->rsp->atv_status.pcl == Si21XXC_ATV_STATUS_RESPONSE_PCL_LOCKED) &&
			(api->rsp->atv_status.dl == Si21XXC_ATV_STATUS_RESPONSE_DL_LOCKED));
}

void DevSi21XXSetScanMode(BOOLEAN bEnable)
{
    scan_mode = bEnable;

    if(scan_mode)
    {
        api->prop->atv_audio_mode.audio_sys = Si21XXC_ATV_AUDIO_MODE_PROP_AUDIO_SYS_WIDE_SCAN;
        api->prop->atv_audio_mode.chan_bw = Si21XXC_ATV_AUDIO_MODE_PROP_CHAN_BW_12X_OVERMOD;
...
void devSetCleanView(BOOLEAN bEnable)
{
    int return_code1 = NO_Si21XXC_ERROR;
    int return_code2 = NO_Si21XXC_ERROR;

    if(bEnable)
    {
    	unsigned int data[] = { 0x0109, 0x4500, 0xE0BF, 0x0002, 0x00A3, 0xAA00, 0x3020, 0x0103, 0x00A6 };

        return_code1 = Si2190B_L1_SSVD_MULTI_REG_WR(api,0,Si2190B_SSVD_REG_WR_CMD_UPDATE_UPDATE,7,data[0],data[1],data[2],data[3],data[4],data[5],data[6]);

    	return_code2 = Si2190B_L1_SSVD_MULTI_REG_WR(api,7,Si2190B_SSVD_REG_WR_CMD_UPDATE_UPDATE,2,data[7],data[8],0,0,0,0,0);
    }
    else
    {
        unsigned int data = 0x0101;

        return_code1 = Si2190B_L1_SSVD_REG_WR(api,0,Si2190B_SSVD_REG_WR_CMD_UPDATE_UPDATE,0,data);
    }
    if(return_code1 != NO_Si21XXC_ERROR || return_code2 != NO_Si21XXC_ERROR)
    {
        printf("[Error]%s, %d, %d, %d\n",__FILE__,__LINE__, return_code1, return_code2);
    }
}
