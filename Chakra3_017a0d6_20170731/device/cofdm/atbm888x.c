...
extern ATBM_I2CREADWRITE_STATUS I2CWrite(uint8 ui8I2CSlaveAddr, uint8 *ui8pData, int i32Length)
 {
     ...
     if(retval == ATBM_I2CREADWRITE_ERROR)
     {
         printf("\r\n I2C Write Fail!!\n");
     }
     return(retval);
...
int ATBMPowerOnInit(void)
{
	...
	stCustomConfig.ui32SampleClkFrequency               = 30400;//24000; //unit: KHz
...
******************************************************************************/
    void ATBMDVBCInit( custom_config_t stCustomConfig_t)
    {
        ...
        ui32SymbolRateK = (uint32)((stCustomConfig_t.stDvbcParams.i32SymbolRate)); 
        ...
        if(ui8ATBMChipID == 0x40)
        {
            ...
            if(stCustomConfig_t.ui32SampleClkFrequency == 30400) //30.4MHz crystal
            {
            ...
            if(stCustomConfig_t.ui32SampleClkFrequency == 24000) //24MHz crystal
            {
                ATBMRead(0x06, 0x0e,&ui8TmpData);
                if(ui8TmpData&0x08)
                {
        ...
        ui32DvbcSymbolRate = stCustomConfig_t.stDvbcParams.i32SymbolRate;
        if(stCustomConfig_t.ui32SampleClkFrequency == 24000) //24MHz crystal
        {
            ui32OSCFreq = stCustomConfig_t.ui32SampleClkFrequency*2;    
        ...
        {
            ui32OSCFreq = stCustomConfig_t.ui32SampleClkFrequency;  
            if(ui8ATBMChipID == 0x30)
            {
        ...
        if(stCustomConfig_t.stDvbcParams.ui8InputMode == DVBC_IF_INPUT)
        {
            ui32IFFreq = stCustomConfig_t.stTunerConfig.ui32IFFrequency;    
...
{ 
	...
	enumRetval = I2CRead(ui8ATBMSlaveAddr, 2, ui8AddrData, 1, ui8pValue);
        ...
        if(enumRetval == ATBM_I2CREADWRITE_ERROR)
            printf("ATBM 888x Read I2C fail!!\n");
...
{ 
	...
	//Option 2: two step format, returned status from low level I2C interface 
                    //should be processed here if needed.  
	if(enumRetval == ATBM_I2CREADWRITE_ERROR)
            printf("ATBM 888x write I2C fail!!\n");
	//the following I2C operations will not be interrupted. 
	//reg_data[0] = ui8Data;
