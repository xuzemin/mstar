...
///// Table
#include "Marlon_Bandwidth_RegTable.h"
#include "Marlon_Bandwidth_RegTable.c"
...
static void _MDrv_BW_DumpTable(TAB_Info* ptab_Info)
{
    ...
    while (1)
    {
        ...
        {
            if ((u16Addr >> 8) == 0x15)  // miu_arb: bank 0x1615
                MApi_XC_WriteByteMask( (MS_U32)u16Addr + 0x160000 ,u8Value,u8Mask);
            else
