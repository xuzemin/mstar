...
#if 0//(FRONTEND_DEMOD_S2_TYPE == MSTAR_MSB131X_DVBS2)
#include "device_demodulator_msb131x.h"
#endif

#define A8304_SLAVE_ADDR_R   0x11
#define A8304_SLAVE_ADDR_W   0x10

#define A8304_SLAVE_ADDR     ((U16)(DVBS_DEMOD_IIC_BUS<<8)|A8304_SLAVE_ADDR_W)

//   A8304  control register
/*
bit7 bit6 0 //default 0
...
4 PNG       Set to 1 when the A8304 is enabled and the LNB output voltage is either too low or too high (nominally ¡À9%
            from the LNB DAC setting). Set to 0 when the A8304 is enabled and the LNB voltage is within the acceptable
            range (nominally ¡À5% from the LNB DAC setting).
5 ¨C        Not used.
6 TSD       The TSD bit is set to 1 if the A8304 has detected an overtemperature condition. If the TSD bit is set to 1, then
...
MS_BOOL MDrv_Dish_Init(void)
{
    ...
    if(!MDrv_IIC_ReadBytes(A8304_SLAVE_ADDR, 0, NULL, 1, &u8Data))
    {
        printf("\n MDrv_Dish_Init failed-1-ReadBytes fail!");
    }
    u8Data = DATA_13V_OUT;
    u8BuffTemp[1]=u8Data;
    if(!MDrv_IIC_WriteBytes(A8304_SLAVE_ADDR, 0, NULL, 2, &u8BuffTemp[0]))
    {
...
MS_BOOL MDrv_Dish_SetLNBPower(DISH_LNBPWR_TYPE enLNBPwr)
{
    ...
    u8BuffTemp[0]=0x00;
      ...
      switch(enLNBPwr)
      {
        ...
        u8BuffTemp[1]=u8Data;
        u8PowerData=u8Data;
        b8result=MDrv_IIC_WriteBytes(A8304_SLAVE_ADDR, 0, NULL, 2, &u8BuffTemp[0]);
        if(0==b8result)
        {
...
MS_BOOL MDrv_Dish_IsOverCurrent(void)
{
    ...
    MDrv_IIC_WriteBytes(A8304_SLAVE_ADDR, 0, NULL, 2, &u8BuffTemp[0]);
    if(MDrv_IIC_ReadBytes(A8304_SLAVE_ADDR, 0, NULL, 1, &u8Status))
    {
