...
char* MDrv_PQ_GetSrcTypeName_2(PQ_WIN eWindow, U16 u16SrcType)
{
    ...
    {
        MS_U32 u32Addr;
        u32Addr = MDrv_PQTextBin_GetSrcType(u16SrcType, &stPQTextBinHeaderInfo[eWindow]);
        return ((char *)u32Addr);
    }
    else
#endif
    {
        ...
        return NULL;
    #else
        return PQ_INPUTTYPE_TEXT_Main[u16SrcType];
    #endif
    }
}
...
char* MDrv_PQ_GetTableName(MS_U8 u8PQIPIdx, MS_U8 u8TabIdx )
{
    ...
    PQ_WIN ePq_Win = PQ_MAIN_WINDOW;

    if( u8TabIdx >= MDrv_PQ_GetTableNum(ePq_Win, u8PQIPIdx) )
    {
        return "TabIdx error";
    }
...
#endif
    {
        ...
        char** ppcIpTextTab = PQ_IPTAB_TEXT_Main[u8PQIPIdx];
        return ppcIpTextTab[u8TabIdx];
    #endif
    }
}
