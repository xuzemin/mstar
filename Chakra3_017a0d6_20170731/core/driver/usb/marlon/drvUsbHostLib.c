...
extern U8 drvUSBHost_PTP_Initial_Port3(void);
extern U8 drvUSBHost_HID_Initial(void);
extern U8 drvUSBHost_HID_Initial_Port2(void);
extern U8 drvUSBHost_HID_Initial_Port3(void);
...
void drvUSBHost_TurnOffPowerDownMode_Port2(void)
{
    ...
    MDrv_Write2Byte(0x103A00+0x20, 0x8051);
    MDrv_Write2Byte(0x103A00+0x22, 0x5088);
...
// For USB Port 3
/////////////////////////////////////////////////////////////////////////////
extern void UsbPort3UTMIInitial(void);

    #ifdef UHC3_BASE
    U32     gUHC3_BASE = UHC3_BASE;
    #else
    U32     gUHC3_BASE = 0;
    #endif

    #ifdef UTMIBaseAddr3
    U32     gUTMI3_BASE= UTMIBaseAddr3;
    #else
    U32     gUTMI3_BASE= 0;
    #endif

    #ifdef USBCBase3
    U32     gUSBC3_BASE = USBCBase3;
    #else
    U32     gUSBC3_BASE = 0;
    #endif

void drvUSBHost_TurnOffPowerDownMode_Port3(void)
{
#if defined(ENABLE_USB_3)
#if (ENABLE_USB_3) && defined(USB_POWER_SAVING_MODE)
    U32 ii;

    printf("USB port 2 exit power save ");

    MDrv_WriteByte( 0x113800, 0x0a);          // Disable MAC initial suspend, Reset UHC
    MDrv_WriteByte( 0x113800, 0x28);          // Release UHC reset, enable UHC and OTG XIU function

    MDrv_Write2Byte(0x103900+0x20, 0x8051);
    MDrv_Write2Byte(0x103900+0x22, 0x5088);
    MDrv_WriteRegBit(0x103900+0x02, ENABLE, BIT2);  // Set bit <2> (CLK12_SEL) to 1 for low voltage crystal clock
    MDrv_Write2Byte((0x103900+0x08), 0x0000); // Disable force_pll_on // Enable band-gap current
    MDrv_Write2Byte(0x103900 , 0x6bc3); // reg_pdn: bit<15>, bit <2> ref_pdn,Turn on reference voltage and regulator
    MsOS_DelayTask(1);         // delay 1ms

    MDrv_Write2Byte(0x103900, 0x69c3); // Turn on UPLL, reg_pdn: bit<9>
    MsOS_DelayTask(2);          // delay 2ms

    MDrv_Write2Byte(0x103900, 0x0001); // Turn all (including hs_current) use override mode

    MDrv_WriteByte(0x113800, 0x0A); // Disable MAC initial suspend, Reset UHC
    MDrv_WriteByte(0x113800, 0x28); // Release UHC reset, enable UHC XIU function

    MDrv_WriteRegBit(0x103900+0x3C, ENABLE, 0x01);  // set CA_START as 1
    MsOS_DelayTask(10);
    MDrv_WriteRegBit(0x103900+0x3C, DISABLE, 0x01);  // release CA_START

    ii=0;
    while( (MDrv_ReadByte(0x103900+0x3C) & 0x02) == 0)  // polling bit <1> (CA_END)
    {
        if (ii++>10000)
        {
            printf("Poll USB port1 CA_END timeout\n");
    ...
    MDrv_WriteByteMask(0x113800+0x02, BIT0, (BIT0|BIT1));  //UHC select enable

    MDrv_WriteRegBit(0x113900+0x40, DISABLE, BIT4); //0: VBUS On.
    MsOS_DelayTask(1);
    printf("done\n");
#endif
#endif
...
void MDrv_USB_ExitPowerSaveMode_Port3(void)
{
    drvUSBHost_TurnOffPowerDownMode_Port3();
}

void drvUSBHost_TurnOnPowerDownMode_Port3(void)
{
#if defined(ENABLE_USB_3)
#if (ENABLE_USB_3) && defined(USB_POWER_SAVING_MODE)
    printf("USB port 2 enter power save ");

    MsOS_DelayTask(2);

    MDrv_WriteRegBit(0x103900+0x08, ENABLE, BIT7); // power down band gap current
    MDrv_WriteByte(0x103900+0x02, 0x80); // bit <15>: reg_hs_rterm_pdn, bit <7>: reg_bond_sel
    MDrv_WriteByte(0x103900+0x03, 0x90);
    MDrv_WriteByte(0x103900, 0xC7); // power down override mode enable
    MDrv_WriteByte(0x103900+0x01, 0xEF); // turn off all except fl_xcvr_pdn

    MDrv_WriteByte(0x113800, 0x00); // Enable initial suspend
    MDrv_WriteByte(0x113800+0x02, 0x00); // UHC select disable
    printf("done\n");
#endif
#endif
}

void MDrv_USB_EnterPowerSaveMode_Port3(void)
{
    drvUSBHost_TurnOnPowerDownMode_Port3();
}

void drvUSBHost_UTMIInitial_Port3(void)
{
#if defined(ENABLE_USB_3)
#if (ENABLE_USB_3)
    UsbPort3UTMIInitial();
...
BOOLEAN drvUSBHost_isPort3Enable(void)
{
#if defined(ENABLE_USB_3)
  #if (ENABLE_USB_3)
    return TRUE;
...
/////////////////////////////////////////////////////////////////////////////
// For Both Port 0, Port 2, and Port 3
/////////////////////////////////////////////////////////////////////////////
...
#else
U8 gUsbDeviceState_Port3=NO_DEVICE;
#endif
...
U8 drvUSBHost_PTP_Init(U8 u8UsbPort)
{
    ...
    if (u8UsbPort == USB_PORT_3)
    {
#if defined(ENABLE_PTP) && defined(USB3_PTP_ENABLE)
        return drvUSBHost_PTP_Initial_Port3();
#endif
    }
...
U8 drvUSBHost_HID_Init(U8 u8UsbPort)
{
   if (u8UsbPort == USB_PORT_0)
    ...
    {
#if defined(ENABLE_USB_3)
#if  (ENABLE_USB_3) && defined(USB3_HID_ENABLE)
        return drvUSBHost_HID_Initial_Port3();
#endif
#endif
    }
    return 1;   //PTP_RESP_FAIL
