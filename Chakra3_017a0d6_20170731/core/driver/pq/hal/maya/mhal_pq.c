...
#include "Maya_Main.h"              // table config parameter
...
{
    MS_U16 i, j, x;
    MS_U8 u8Ramcode[8] = {0}; // Fix coverity
    MS_U32 u32Addr;
    MS_U8 u8RamCount = 8;
    ...
    // SRAM1: 5x8bits for each data
    // SRAM2: 8x8bits for each data
    // SRAM3: 5x8bits for each data
    // SRAM4: 8x8bits for each data
    ...
    for ( i=0; i<64; i++)
    {
        ...
        else
        {
            ...
            MApi_XC_W2BYTEMSK(REG_SC_BK23_49_L, (((MS_U16)u8Ramcode[7])<<8|(MS_U16)u8Ramcode[6]), 0x0FFF);
        }
...
MS_U16 Hal_PQ_get_hnonlinear_idx(MS_HNONLINEAR_TYPE etype)
{
    ...
    case MS_HNONLINEAR_3840_3:
        u16ret = PQ_IP_HnonLinear_H_3840_3_Main;
        break;

    case MS_HNONLINEAR_3840_4:
        u16ret = PQ_IP_HnonLinear_H_3840_4_Main;
        break;
...
MS_U16 Hal_PQ_get_madi_idx(MS_BOOL bMainWin, MS_MADI_TYPE etype)
{
    ...
    case MS_MADI_25_8F_4R_MC:
        u16ret = PQ_IP_MADi_25_8F_4R_MC_Main;
        break;
...
#if PQ_ENABLE_RFBL
    case MS_MADI_RFBL_2D:
        u16ret = PQ_IP_MADi_RFBL_2D_Main;
        break;
    ...
    case MS_MADI_P_MODE_RY8:
        u16ret = PQ_IP_MADi_RY8_MODE_Main;
        break;
...
MS_U16 Hal_PQ_get_sram_size(MS_U16 u16sramtype)
{
    ...
    case PQ_TABTYPE_XVYCC_DE_GAMMA_SRAM:
        u16ret = PQ_IP_XVYCC_DE_GAMMA_SRAM_SIZE_Main;
        break;
    case PQ_TABTYPE_XVYCC_GAMMA_SRAM:
        u16ret = PQ_IP_XVYCC_GAMMA_SRAM_SIZE_Main;
        break;
    default:
        u16ret = 0;
        break;
...
MS_U8 Hal_PQ_get_madi_fbl_mode(MS_BOOL bMemFmt422,MS_BOOL bInterlace)
{
    ...
    if(bInterlace)
    {
        return MS_MADI_RFBL_2D;
    }
    else
    {
        if(bMemFmt422)
        {
            return MS_MADI_RFBL_P_MODE10_MOT;
        }
        else
        {
            return MS_MADI_RFBL_P_MODE8_MOT;
        }
    }
}
...
void Hal_PQ_set_sram_xvycc_gamma_table(MS_U8 u8sramtype, void *pData)
{
    ...
    if (u8sramtype == SC_FILTER_SRAM_XVYCC_DE_GAMMA)
    {
        ...
        MS_U16 u16OffsetCount1=0;
        MS_U16 u16OffsetCount2=0;
        MS_U16 u16OffsetCount3=0;
        MS_U16 u16OffsetCount4=0;
        MS_U16 u16OffsetData=0;
        ...
        for( i = 0; i < PQ_IP_XVYCC_DE_GAMMA_SRAM_SIZE_Main; i += 4 )
        {
            u16InputData1 = ((MS_U16)u8Addr[i+1]<<8)|((MS_U16)u8Addr[i]);
            u16InputData2 = ((MS_U16)u8Addr[i+3]<<8)|((MS_U16)u8Addr[i+2]);
            u16InputData3 = ((MS_U16)u8Addr[i+5]<<8)|((MS_U16)u8Addr[i+4]);

            u32Ref = u16InputData1&0x7FFF; // 15bis
            u32Delta1 = (u16InputData2 - u16InputData1);
            u32Delta2 = (u16InputData3 - u16InputData2);

            if (u32Delta1<32)
                u16OffsetCount1= u16OffsetCount1+2;
            else if (u32Delta1>=32 && u32Delta1<64)
                u16OffsetCount2= u16OffsetCount2+2;
            else if (u32Delta1>=64 && u32Delta1<96)
                u16OffsetCount3= u16OffsetCount3+2;
            else if (u32Delta1>=96)
                u16OffsetCount4= u16OffsetCount4+2;

            u32Delta1=((u32Delta1>0x007F)?0x007F:u32Delta1)&0x1F;
            u32Delta2=((u32Delta2>0x007F)?0x007F:u32Delta2)&0x1F;
        ...
        MApi_XC_W2BYTE(REG_SC_BK2D_60_L, 0 ); // Disable Write
            //Write Offset Value
        MApi_XC_W2BYTE(REG_SC_BK2D_66_L, u16OffsetCount1); //data
        MApi_XC_W2BYTE(REG_SC_BK2D_69_L, u16OffsetCount1); //data
        MApi_XC_W2BYTE(REG_SC_BK2D_6C_L, u16OffsetCount1); //data
        MApi_XC_W2BYTE(REG_SC_BK2D_67_L, u16OffsetCount1+u16OffsetCount2); //data
        MApi_XC_W2BYTE(REG_SC_BK2D_6A_L, u16OffsetCount1+u16OffsetCount2); //data
        MApi_XC_W2BYTE(REG_SC_BK2D_6D_L, u16OffsetCount1+u16OffsetCount2); //data
        MApi_XC_W2BYTE(REG_SC_BK2D_68_L, u16OffsetCount1+u16OffsetCount2+u16OffsetCount3); //data
        MApi_XC_W2BYTE(REG_SC_BK2D_6B_L, u16OffsetCount1+u16OffsetCount2+u16OffsetCount3); //data
        MApi_XC_W2BYTE(REG_SC_BK2D_6E_L, u16OffsetCount1+u16OffsetCount2+u16OffsetCount3); //data
        u16OffsetData=0;
        if (u16OffsetCount2>=1) u16OffsetData |= 0x0004;
        if (u16OffsetCount3>=1) u16OffsetData |= 0x0020;
        if (u16OffsetCount4>=1) u16OffsetData |= 0x00C0;
        MApi_XC_W2BYTE(REG_SC_BK2D_6F_L,u16OffsetData);
        MApi_XC_W2BYTE(REG_SC_BK2D_70_L,u16OffsetData);
        MApi_XC_W2BYTE(REG_SC_BK2D_71_L,u16OffsetData);
    }
    else
    {
...
void Hal_PQ_Set2StepScaling(MS_BOOL bEnable)
{
    ...
    if(bEnable)
    {
        MApi_XC_W2BYTEMSK(REG_SC_BK4F_10_L, BIT(0), BIT(0));
        MApi_XC_W2BYTEMSK(REG_SC_BK18_75_L, 0, BIT(2));
    }
    else
    {
        MApi_XC_W2BYTEMSK(REG_SC_BK4F_10_L, 0, BIT(0));
        MApi_XC_W2BYTEMSK(REG_SC_BK18_75_L, BIT(2), BIT(2));
    }
#endif
}
