...
unsigned char Si2158_pollForCTS (L1_Si2158_Context *api)
{
  ...
  start_time = system_time_si2158();

  while (system_time_si2158() - start_time <1000)  { /* wait a maximum of 1000ms */
    if (Si2158_L0_ReadCommandBytes(api->i2c, 1, rspByteBuffer) != 1) {
...
unsigned char Si2158_pollForResponse (L1_Si2158_Context *api, unsigned int nbBytes, unsigned char *pByteBuffer)
{
  ...
  while (system_time_si2158() - start_time <1000)  { /* wait a maximum of 1000ms */
    if ((unsigned int)Si2158_L0_ReadCommandBytes(api->i2c, nbBytes, pByteBuffer) != nbBytes) {
                                         ...
                                         unsigned char   force_top_gain)
{
    ...
    if (Si2158_L0_WriteCommandBytes(api->i2c, 2, cmdByteBuffer) != 2) {
 ...
 /*---------------------------------------------------*/
/* Si2158_APPLY_SAMPLE_FREQ COMMAND                */
/*---------------------------------------------------*/
unsigned char Si2158_L1_APPLY_SAMPLE_FREQ (L1_Si2158_Context *api)
{
    ...
    unsigned char rspByteBuffer[1];
    api->rsp->apply_sample_freq.STATUS = api->status;

    SiTRACE("Si2158 APPLY_SAMPLE_FREQ ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158_APPLY_SAMPLE_FREQ_CMD;

    if (Si2158_L0_WriteCommandBytes(api->i2c, 1, cmdByteBuffer) != 1) {
      SiTRACE("Error writing APPLY_SAMPLE_FREQ bytes!\n");
      return ERROR_Si2158_SENDING_COMMAND;
    }

    error_code = Si2158_pollForResponse(api, 1, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling APPLY_SAMPLE_FREQ response\n");
      return error_code;
    }
                                         ...
                                         unsigned char   en_xout)
{
    ...
    if (Si2158_L0_WriteCommandBytes(api->i2c, 3, cmdByteBuffer) != 3) {
                                         ...
                                         unsigned char   reserved3)
{
    ...
    if (Si2158_L0_WriteCommandBytes(api->i2c, 6, cmdByteBuffer) != 6) {
                                                   ...
                                                   int   offset_500hz)
{
    ...
    if (Si2158_L0_WriteCommandBytes(api->i2c, 4, cmdByteBuffer) != 4) {
                                         ...
                                         unsigned char   reserved8)
{
    ...
    if (Si2158_L0_WriteCommandBytes(api->i2c, 10, cmdByteBuffer) != 10) {
                                         ...
                                         unsigned char   wake_up)
{
    ...
    if (Si2158_L0_WriteCommandBytes(api->i2c, 15, cmdByteBuffer) != 15) {
                                         ...
                                         unsigned long   freq)
{
    ...
    SiTRACE("Si2158_L1_TUNER_TUNE_FREQ %ld\n",freq);
    if (Si2158_L0_WriteCommandBytes(api->i2c, 8, cmdByteBuffer) != 8) {
...
unsigned char   Si2158_L1_SendCommand2(L1_Si2158_Context *api, unsigned int cmd_code) {
    ...
    #endif /*     Si2158_AGC_OVERRIDE_CMD */
    #ifdef        Si2158_APPLY_SAMPLE_FREQ_CMD
     case         Si2158_APPLY_SAMPLE_FREQ_CMD_CODE:
       return Si2158_L1_APPLY_SAMPLE_FREQ (api );
     break;
    #endif /*     Si2158_APPLY_SAMPLE_FREQ_CMD */
    #ifdef        Si2158_ATV_CW_TEST_CMD
...
unsigned char   Si2158_L1_GetCommandResponseString(L1_Si2158_Context *api, unsigned int cmd_code, char *separator, char *msg) {
    ...
    #ifdef        Si2158_APPLY_SAMPLE_FREQ_CMD
     case         Si2158_APPLY_SAMPLE_FREQ_CMD_CODE:
      sprintf(msg,"APPLY_SAMPLE_FREQ ");
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-TUNINT ");
           if  (api->rsp->apply_sample_freq.STATUS->tunint ==     0) strcat(msg,"NOT_TRIGGERED");
      else if  (api->rsp->apply_sample_freq.STATUS->tunint ==     1) strcat(msg,"TRIGGERED    ");
      else                                                          sprintf(msg,"%s%d", msg, api->rsp->apply_sample_freq.STATUS->tunint);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-ATVINT ");
           if  (api->rsp->apply_sample_freq.STATUS->atvint ==     0) strcat(msg,"NOT_TRIGGERED");
      else if  (api->rsp->apply_sample_freq.STATUS->atvint ==     1) strcat(msg,"TRIGGERED    ");
      else                                                          sprintf(msg,"%s%d", msg, api->rsp->apply_sample_freq.STATUS->atvint);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-DTVINT ");
           if  (api->rsp->apply_sample_freq.STATUS->dtvint ==     0) strcat(msg,"NOT_TRIGGERED");
      else if  (api->rsp->apply_sample_freq.STATUS->dtvint ==     1) strcat(msg,"TRIGGERED    ");
      else                                                          sprintf(msg,"%s%d", msg, api->rsp->apply_sample_freq.STATUS->dtvint);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-ERR    ");
           if  (api->rsp->apply_sample_freq.STATUS->err    ==     1) strcat(msg,"ERROR   ");
      else if  (api->rsp->apply_sample_freq.STATUS->err    ==     0) strcat(msg,"NO_ERROR");
      else                                                          sprintf(msg,"%s%d", msg, api->rsp->apply_sample_freq.STATUS->err);
       sprintf(msg,"%s%s",msg,separator); strcat(msg,"-CTS    ");
           if  (api->rsp->apply_sample_freq.STATUS->cts    ==     1) strcat(msg,"COMPLETED");
      else if  (api->rsp->apply_sample_freq.STATUS->cts    ==     0) strcat(msg,"WAIT     ");
      else                                                          sprintf(msg,"%s%d", msg, api->rsp->apply_sample_freq.STATUS->cts);
     break;
    #endif /*     Si2158_APPLY_SAMPLE_FREQ_CMD */
