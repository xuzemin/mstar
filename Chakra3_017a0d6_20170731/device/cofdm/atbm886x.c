...
Description: Description: Software development source codes for ATBM886x DTMB/DVBC demodulator
Version: 1.0.1
Released 2012-09-07
...
extern ATBM_I2CREADWRITE_STATUS I2CWrite(uint8 ui8I2CSlaveAddr, uint8 *ui8pData, int i32Length)
 {
   /* I2C write, please port this function*/
    ATBM_I2CREADWRITE_STATUS retval = ATBM_I2CREADWRITE_OK;
    retval=(ATBM_I2CREADWRITE_STATUS)MDrv_IIC_WriteBytes(((U16)DEMOD_IIC_BUS<<8)|ui8I2CSlaveAddr,i32Length,ui8pData,0,NULL);
    if(!retval)
    {
        printf("\r\n I2C Write Fail!!\n");
    }
...
extern ATBM_I2CREADWRITE_STATUS I2CRead(uint8 ui8I2CSlaveAddr, uint8 *ui8pData, int i32Length)
 {
   /* I2C write, please port this function*/
    ATBM_I2CREADWRITE_STATUS retval = ATBM_I2CREADWRITE_OK;
    retval=(ATBM_I2CREADWRITE_STATUS)MDrv_IIC_ReadBytes(((U16)DEMOD_IIC_BUS<<8)|ui8I2CSlaveAddr,0,NULL,i32Length,ui8pData);
    if(!retval)
    {
        printf("\r\n I2C Read Fail!!\n");
    }
...
extern ATBM_I2CREADWRITE_STATUS I2CReadOneStep(uint8 ui8I2CSlaveAddr, uint16 addr_length, uint8 *addr_dat,  uint16 data_length, uint8 *reg_dat)
{
 /*I2C read in one step format, please port this function if needed*/	
    ATBM_I2CREADWRITE_STATUS retval = ATBM_I2CREADWRITE_OK;
    retval=(ATBM_I2CREADWRITE_STATUS)MDrv_IIC_ReadBytes(((U16)DEMOD_IIC_BUS<<8)|ui8I2CSlaveAddr,addr_length,addr_dat,data_length,reg_dat);
    if(!retval)
    {
        printf("\r\n I2C Read Fail!!\n");
    }
...
extern ATBM_I2CREADWRITE_STATUS I2CWriteWithRestart(uint8 ui8I2CSlaveAddr, uint8 addr_length, uint8 *addr_dat,  uint8 data_length, uint8 *reg_dat)
{
 /*I2C write in one step format, please port this function if needed*/	
    ATBM_I2CREADWRITE_STATUS retval = ATBM_I2CREADWRITE_OK;
    retval=(ATBM_I2CREADWRITE_STATUS)MDrv_IIC_WriteBytes(((U16)DEMOD_IIC_BUS<<8)|ui8I2CSlaveAddr,addr_length,addr_dat,data_length,reg_dat);
    if(!retval)
    {
        printf("\r\n I2C Write Fail!!\n");
    }
...
int ATBMPowerOnInit()
{
	...
	stCustomConfig.stTunerConfig.ui8DTMBBandwithMHz     = 8;
	stCustomConfig.stTunerConfig.dbIFFrequency          = 5;

	//demod config
	stCustomConfig.stTsMode.ui8TSTransferType           = TS_SERIAL_MODE;	  //TS_SERIAL_MODE;//
	...
	stCustomConfig.stTsMode.ui8TSSSIOutputSelection     = TS_SSI_OUTPUT_ON_DATA_BIT0;//TS_SSI_OUTPUT_ON_DATA_BIT0;
...
{   
	...
	dbFrameError = ATBMFrameErrorRatio();
       ...
       //printf("ui8TPSCode = %d, ui8TSSync = %d\n", ui8TPSCode, ui8TSSync);
	i32SNR100per = 20;  
	...
	else if((ui8TPSCode <17)&&(ui8TPSCode>10)) /*For 16QAM FEC:0.6 0.8*/
	{
	       //printf("16QAM\n");
		if(i32SNR>100)
		{
	...
	else if((ui8TPSCode <25)&&(ui8TPSCode>16))/*64QAM FEC:0.6; 32QAM 0.8*/
	{
              //printf("64QAM\n");
		if(i32SNR>100)
		{
	...
	if(i32SNR100per>99)
	{
       ...
       if (TRUE != ATBMLockedFlag())
            return 0;
	//printf("i32SNR100per = %d\n", i32SNR100per);
	return i32SNR100per;
}
...
uint16 ATBMSignalStrength_reference(void)	
{
	...
	uint16 AGC_A = 498; //about AGC 0.813V, Signal strength:100
	uint16 AGC_B = 750; //about AGC 1.075V, Signal strength:60
	uint16 AGC_C = 1023; //about AGC 1.351V, Signal strength:20
	...
	/*Tuner Reference evaluation*/
       if (TRUE == ATBMLockedFlag()) // must set locked check
	{
        	if(agc_value <=AGC_A)
        	{
            		signal_percent = 100;	
        	...
        	{   	
            		signal_percent = (uint16)((AGC_B - agc_value)*40/(AGC_B - AGC_A) + 60);	
        	}
...
******************************************************************************/
void ATBMDVBCInit( custom_config_t _stCustomConfig)
{
	...
	if(_stCustomConfig.stDvbcParams.ui8InputMode == DVBC_IF_INPUT)
	{
	...
	dbSymbolRateM = (double)(_stCustomConfig.stDvbcParams.i32SymbolRate*1.0)/1000; 
	...
	if(ui8ATBMChipID == 0x40)
	{
		...
		if(((_stCustomConfig.dbSampleClkFrequency-30.4)<0.00001)&&((_stCustomConfig.dbSampleClkFrequency-30.4)>-0.00001)) //30.4MHz crystal
		{
		...
		if(((_stCustomConfig.dbSampleClkFrequency-24)<0.00001)&&((_stCustomConfig.dbSampleClkFrequency-24)>-0.00001)) //24MHz crystal
		{
			ATBMRead(0x06, 0x0e,&ui8TmpData);
			if(ui8TmpData&0x08)
			{
	...
	if(((_stCustomConfig.dbSampleClkFrequency-24)<0.00001)&&((_stCustomConfig.dbSampleClkFrequency-24)>-0.00001)) //24MHz crystal
	{
		dbOSCFreq = _stCustomConfig.dbSampleClkFrequency*2;	
	...
	{
		dbOSCFreq = _stCustomConfig.dbSampleClkFrequency;	
		if(ui8ATBMChipID == 0x30)
		{
	...
	if(_stCustomConfig.stDvbcParams.ui8InputMode == DVBC_IF_INPUT)
	{
		dbIFFreq = _stCustomConfig.stTunerConfig.dbIFFrequency;	
...
{  
    ...
    if(ATBM_I2CREADWRITE_OK == ATBMRead(0x0d,0x04,&ui8TPS))
        {
              //printf("ATBMGetTPS 11111111\n");
		return ui8TPS;
        }
	else
        {   
              //printf("ATBMGetTPS 222222222\n");
		return 0xff;
       }
}
...
void ATBMSetClock64M(void)
{
	...
	ui8Rst =(uint8)floor(3.4375/dbDvbcSymbolRate*64/2 + 0.5); 
	ATBMWrite(0x15, 0x37, ui8Rst);
	Delayms(2);
...
void ATBMSetClock72M(void)
{
	...
	ui8Rst =(uint8)floor(3.4375/dbDvbcSymbolRate*72/2 + 0.5); 
	ATBMWrite(0x15, 0x37, ui8Rst);
	Delayms(2);
	ATBMWrite(0x01, 0x0c, 1); 
}
