...
void devTunerSetPLLData(WORD wPLLData, RFBAND eBand)
{
    ...
    cTunerData[2] = 0x80|DIVIDER_RATIO|CHARGE_PUMP_50uA;
	cTunerData[3]=0x04;
    ...
    cTunerData[4]  = cTunerData[2]&0xc7;
    cTunerData[4] |= 0x18;
    cTunerData[5] = IF_OUT_LEVEL_Extenrnal;
...
void devDigitalTuner_SetFreq ( double Freq, RF_CHANNEL_BANDWIDTH eBandWidth)
{
    ...
    //#if (FRONTEND_DEMOD_TYPE == MSTAR_MSB1210_DEMOD)    // GEM_SYNC_0815
    ...
    #if (FRONTEND_DEMOD_TYPE == MSTAR_MSB1210_DEMOD)    // GEM_SYNC_0815
    CONFIG[2] = 0x80 |DIVIDER_RATIO_166_7KHz; // CB1
    #else
    CONFIG[2] = 0x80 | DIVIDER_RATIO_166_7KHz  ;// CB1
    #endif
    ...
    if(Freq <= 174)
    {
        if ( Freq <= 118 )
        {
                CONFIG[2] |= CHARGE_PUMP_50uA;
        }
        else if ( Freq <= 139)
        {
             CONFIG[2] |= CHARGE_PUMP_125uA;

        }
	else
	{
	 CONFIG[2] |= CHARGE_PUMP_250uA;
		}
	...
	{
	 CONFIG[2] |= CHARGE_PUMP_650uA;
		}
	CONFIG[3] |= BAND_SWITCH_UHF;
    }

    CONFIG[4] = CONFIG[2] &0xc7;
    CONFIG[4] |= 0x18;
    CONFIG[5] = IF_OUT_LEVEL_112;
