...
//	func = Si2158_POWER_UP_CMD_FUNC_NORMAL; //power up from device memory
	func = Si2158_POWER_UP_CMD_FUNC_BOOTLOADER; //power up from patch
	...
	if(func != Si2158_POWER_UP_CMD_FUNC_NORMAL)
	{
    ...
    if (api->rsp->part_info.romid == 0x31) {
      /* Load the Firmware */
       if ((return_code = Si2158_LoadFirmware(api, Si2158_FW_0_Bb6, FIRMWARE_LINES_0_Bb6)) != NO_Si2158_ERROR) {
...
int Si2158_LoadFirmware        (L1_Si2158_Context *api, unsigned char fw_table[], int nbLines)
{
    int return_code;
    int line;
//	char cBuffer[32];	/* LGIT Dialog에 진행사항 보여주기 위해  */
    return_code = NO_Si2158_ERROR;
...
#if 0
		//lgit
		sprintf(cBuffer, "Load : %d\% ", (((line+1)*100)/nbLines));
		if( !((((line+1)*1000) / (nbLines)) % 10 ))
			SendMessageToDlg("Si2178", cBuffer);
#endif
    }
    SiTraceConfiguration("traces resume");
 ...
 int  Si2158_Tune              (L1_Si2158_Context *api, unsigned char mode, unsigned long freq)
{
    ...
    while ( (system_time_si2158() - start_time) < timeout )
    {
...
void Send2158Command(L1_Si2158_Context *Si2158,unsigned char ucCmd, unsigned char *ucArg, unsigned char ucLen)
{
	...
	for(i = 0; i<ucLen; i++)
	{
		ucBuffer[i+1] = ucArg[i];
	}
	// LGIT_I2cWrite(Si2158->i2c->address, ucBuffer, ucLen+1);
	LGIT_bsp_i2c_write(Si2158->i2c->address, ucBuffer, ucLen+1);
	
	for(i=0; i<100; i++){
		// LGIT_I2cRead(Si2158->i2c->address, &ucSts, 1);
		LGIT_bsp_i2c_read(Si2158->i2c->address, &ucSts, 1);        
		if(ucSts & 0x80){
			break;
		}
	}
