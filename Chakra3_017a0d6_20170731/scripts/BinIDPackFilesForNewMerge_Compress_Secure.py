...
CHUNK_HEADER_SIZE = 0x400  # The size of chunk header is 128 bytes.
...
def doPack(bin_align, file_align, magic, dst_fn, binid_fn, ImgInfo_Offset, ap_c_fn, compress_format):
    ...
    f.seek((BOOTLOADER_SIZE + BOOTLOADER_OFFSET + ImgInfo_Offset + 0x47 + 0x380))
    ...
    len_of_AP_bytes = struct.pack("BBBB",
                        ...
                        (len_of_AP >> 16)  & 0xFF,
                        (len_of_AP >> 24)  & 0xFF)
    f.seek((BOOTLOADER_SIZE + BOOTLOADER_OFFSET + ImgInfo_Offset + 0x4A + 0x380))
    ...
    f.seek((BOOTLOADER_SIZE + BOOTLOADER_OFFSET + ImgInfo_Offset + 0x4E + 0x380))
    f.write(magic_end_addr_bytes)
    print "magic_end_addr 0x%X" % (magic_end_addr)

    ap_offset = BOOTLOADER_SIZE + PM_CODE_SIZE + BOOTLOADER_OFFSET
    ...
    f.seek(0)
    f.read()
    #f.seek((BOOTLOADER_SIZE + ImgInfo_Offset + 0x4E))
