...
MAPI_U8 MSB1233C_LIB_U01[]={
#include "msb1233c_dvbt.dat"
};
MAPI_U8 MSB1233C_LIB_U02[]={
#include "msb1233d_dvbt.dat"
};
...
EN_MSB_1233C_LOCK_STATUS DTV_DVB_T_GetLockStatus(void);
...
{
	BOOLEAN bRet = 0;
    bRet = MDrv_IIC_WriteBytes(DEMOD_DYNAMIC_SLAVE_ID_MSB_1233_1, u8AddrSize, pu8Addr, u16Size, pu8Data);
	return bRet;
}
...
{
	BOOLEAN bRet = 0;
    bRet = MDrv_IIC_ReadBytes(DEMOD_DYNAMIC_SLAVE_ID_MSB_1233_1, u8AddrSize, pu8Addr, u16Size, pu8Data);
	return bRet;
}
...
{
	BOOLEAN bRet = 0;
    bRet = MDrv_IIC_WriteBytes(DEMOD_DYNAMIC_SLAVE_ID_MSB_1233_2, u8AddrSize, pu8Addr, u16Size, pu8Data);
	return bRet;
}
...
{
	BOOLEAN bRet = 0;
    bRet = MDrv_IIC_ReadBytes(DEMOD_DYNAMIC_SLAVE_ID_MSB_1233_2, u8AddrSize, pu8Addr, u16Size, pu8Data);
	return bRet;
}
...
MAPI_U8 u8MSB1233CChipRev = 0;
MAPI_U8* MSB1233C_LIB = MSB1233C_LIB_U01;
MAPI_U32 u32SizeOfMSB1233C_LIB = sizeof(MSB1233C_LIB_U01);
...
static MAPI_BOOL IspCheckVer(MAPI_U8* pLibData, MAPI_BOOL* pMatch)
{
    ...
    *pMatch = MAPI_TRUE;
    ...
    MAPI_U8    bAddr[1], bError = MAPI_TRUE;
    //MAPI_U16   Count;
...
MAPI_BOOL Load2Sdram(MAPI_U8 *u8_ptr, MAPI_U16 data_length, MAPI_U16 sdram_win_offset_base)
{
    ...
    MAPI_U16 i = 0, j = 0, u16_index = 0;
    MAPI_U8 buf[SPI_DEVICE_BUFFER_SIZE] = {0};

    if((data_length % SPI_DEVICE_BUFFER_SIZE) == 0)
        u16_index = data_length / SPI_DEVICE_BUFFER_SIZE;
    else
        u16_index = data_length / SPI_DEVICE_BUFFER_SIZE + 1;
...
MAPI_BOOL LoadDspCodeToSRAM_dvbt(void)
{
    ...
    {
        // dvbt code
        if(u32SizeOfMSB1233C_LIB > EDINBURGH_DVBT_START_ADDR)
        {
...
MAPI_BOOL LoadDspCodeToSDRAM_Boot(void)
{
    ...
    {
        if(u32SizeOfMSB1233C_LIB > EDINBURGH_BOOT_START_ADDR)
        {
            // boot code
...
MAPI_BOOL LoadDspCodeToSDRAM_dvbt2(void)
{
    ...
    {
        if(u32SizeOfMSB1233C_LIB > EDINBURGH_DVBT2_P1_START_ADDR)
        {
            // dvbt2_p2
...
MAPI_BOOL LoadDspCodeToSDRAM_dvbc(void)
{
    ...
    {
        // dvbc code
        if(u32SizeOfMSB1233C_LIB > EDINBURGH_DVBC_START_ADDR)
        {
...
static MAPI_BOOL IspProcFlash(MAPI_U8* pLibArry, MAPI_U32 dwLibSize)
{
    ...
    //MAPI_U16    wLoop;
    MAPI_U8     bError = MAPI_FALSE;//, bWriteData[PAGE_WRITE_SIZE];
...
//Disable WatchDog, cause this Initialization may cost a long time and reset WatchDog repeatedly
#if (WATCH_DOG == ENABLE )
    ...
    if(g_WO_SPI_FLASH == 1)
    {
        ...
        status &= ReadReg(0x0900+0x01*2, &u8MSB1233CChipRev);
        if (u8MSB1233CChipRev == 0)
        {
            MSB1233C_LIB = MSB1233C_LIB_U01;
            u32SizeOfMSB1233C_LIB = sizeof(MSB1233C_LIB_U01);
        }
        else
        {
            MSB1233C_LIB = MSB1233C_LIB_U02;
            u32SizeOfMSB1233C_LIB = sizeof(MSB1233C_LIB_U02);
        }
    ...
    else
    {
        ...
        do
        {
            ...
            crc16 = MSB1233C_LIB[u32SizeOfMSB1233C_LIB-2];
            crc16 = (crc16<<8)|MSB1233C_LIB[u32SizeOfMSB1233C_LIB-1];

            {
                ...
                if (bMatch == MAPI_FALSE)
                {
                  ...
                  if(msb1233c_flash_WP(0) == MAPI_FALSE)
                  {
...
#endif
                ////bMatch = MAPI_TRUE; //FIXME : Remove this to enable auto FW reload.
                if(bMatch == MAPI_FALSE) // Version or crc not match
                {
                    MAPI_U8     bAddr[1];
                    MAPI_U8 bWriteData[5] = {0x4D, 0x53, 0x54, 0x41, 0x52};

                    printf("*****T2 DEMOD SPI FLASH DOWNLOAD START****\n");
                    if ( (u32SizeOfMSB1233C_LIB%256) != 0)
                    {
                        printf(" MSB1233C_LIB 256byte alignment warning!%lu \n",u32SizeOfMSB1233C_LIB);
                    }
                    ...
                    if ( (u32SizeOfMSB1233C_LIB - 2) > MAX_MSB1233C_LIB_LEN)
                    {
                      printf("Err, msb1233c_lib size(%ld) is larger than flash size(%d)\n",u32SizeOfMSB1233C_LIB,MAX_MSB1233C_LIB_LEN);
                    }

                    // if (IspProcFlash(MSB1233C_LIB, u32SizeOfMSB1233C_LIB) == MAPI_FALSE)
                    if (IspProcFlash(MSB1233C_LIB, u32SizeOfMSB1233C_LIB-2) == MAPI_FALSE)
                    {
                    ...
                    else
                    {
                        ...
                        //check again
                        if((IspCheckVer(MSB1233C_LIB, &bMatch) == MAPI_FALSE) || (bMatch == MAPI_FALSE))
                        {
                ...
                if (bMatch == MAPI_TRUE)
                {
                  ...
                  if(msb1233c_flash_WP(1) == MAPI_FALSE)
                  {
    ...
    ExtendCmd(0x81, 0, 0, NULL); //tri-state in default
...
MAPI_BOOL ExtendCmd(MAPI_U8 SubCmd, MAPI_U32 u32Param1, MAPI_U32 u32Param2, void *pvParam3)
{
    ...
    if (SubCmd == 0x84)
    {
        ...
        // freeze
        u8Status &= ReadReg(0x0b00+0x28*2+1, &u8_freeze);
        u8Status &= WriteReg(0x0b00+0x28*2+1, u8_freeze|0x80);

        u8Status &= ReadDspReg((MAPI_U16)E_T2_TOTAL_CFO_1, &u8Data1);
        u16Data = u8Data1;
        u8Status &= ReadDspReg((MAPI_U16)E_T2_TOTAL_CFO_0, &u8Data1);
        u16Data = (u16Data<<8)|u8Data1;

        // unfreeze
        u8Status &= WriteReg(0x0b00+0x28*2+1, u8_freeze);

        s16_cfo10 = (MS_S16)u16Data;
        ...
        switch(m_enCurrentDemodulator_Type)
        {
            case E_DEVICE_DEMOD_DVB_T2:
            {
                ...
                if(bCheckPass)
                {
                    ...
                    bDoReset = MAPI_FALSE;
                    *((EN_MSB_1233C_LOCK_STATUS *)pvParam3) = E_MSB_1233C_DEMOD_LOCK;
                }
                else if((u32NowTime - u32ChkScanTimeStart < u32Timeout)
                        || (u32NowTime - u32LockTimeStartDVBT2 < u32LockTimeout))
                {
                    FECLock = MAPI_FALSE;
                    *((EN_MSB_1233C_LOCK_STATUS *)pvParam3) = E_MSB_1233C_DEMOD_CHECKING;
                }
                else
                {
                    FECLock = MAPI_FALSE;
                    *((EN_MSB_1233C_LOCK_STATUS *)pvParam3) = E_MSB_1233C_DEMOD_UNLOCK;
                }
            }
            break;
        ...
        case E_DEVICE_DEMOD_DVB_T2:
        {
            ...
            else
            {
                ...
                if (u8_win == 1)
                {
                    ...
                    status &= ReadReg(0x2700+0x31*2, &u8_reg);
                    u8_gi = (u8_reg>>1)&0x07;
...
MAPI_BOOL DTV_DVB_T_Get_TPS_Parameter(MAPI_U16 * pu16TPS_parameter, E_SIGNAL_TYPE eSignalType)
{
    ...
    if(DTV_DVB_T_GetLockStatus() == E_MSB_1233C_DEMOD_LOCK)
    {
...
#define DVBT2_BER_TH_HY 0.1
    ...
    switch(m_enCurrentDemodulator_Type)
    {
        case E_DEVICE_DEMOD_DVB_T2:
        {
            ...
            if(DTV_GetPostBER(&fber) == false)
            {
                return 0;
            }

            DBG_DEMOD_MSB(printf("MSB1233C Data BER = %8.3e \n ", fber));
...
MAPI_U16 DTV_GetSignalStrength(void)
{
    ...
    static MAPI_U8 u8SignalEverStable=MAPI_FALSE, u8ModulationStable=0, u8CodeRateStable=0;
    ...
    // retb &= device_tuner_get_rssi(&fPinput);
    retb &= mdev_GetSignalStrength(&fPinput);
    IIC_Bypass_Mode(MAPI_FALSE);

    retb &= ReadReg(0x0990, &reg);
    demodState = reg;

    retb &= ReadReg(0x2700 + (0x47 * 2), &u8Modulation);
    u8Modulation=(((MAPI_U16) u8Modulation) & (BIT5 | BIT4 | BIT3)) >> 3;
    if (u8Modulation>=3) u8Modulation=3;
    retb &= ReadReg(0x2700 + (0x47 * 2), &u8CodeRate);
    ...
    if ((reg >= 0x09)   &&   (demodState >= 0x09))
    {
        fPrel=fPinput-fDVBT2_SSI_Pref[u8Modulation][u8CodeRate];
        if (u8Modulation || u8CodeRate) // in case context switch or else...
        {
            u8ModulationStable = u8Modulation;
            u8CodeRateStable = u8CodeRate;
            u8SignalEverStable = MAPI_TRUE;
        }
    }
        ...
        )
    {
        ...
        if (f_ssi > 100) u8_ssi = 100;
        else if (f_ssi < 0) u8_ssi = 0;
        else u8_ssi = (MAPI_U8)(f_ssi+0.5);

        if (u8SignalEverStable == MAPI_FALSE) u8_ssi = 0;

        // printf("MJ_SSI... RF_level=%f, f_ssi=%f,ssi=%d,cr=%d,mod=%d\n",fPinput,f_ssi,ssi,u8CodeRateStable,u8ModulationStable);
    }
    else
    {
        if (fPrel<-15.0)
        ...
        {
            u8_ssi = (MAPI_U8)(((double)2.0/3)*(fPrel+15.0));
        }
        else if (fPrel<20.0)
        {
            u8_ssi = 4*fPrel+10;
        ...
        {
            u8_ssi = (MAPI_U8)(((double)2.0/3)*(fPrel-20.0)+90);
        }
        else
        {
            u8_ssi = 100;
        }
        // printf("old ssi, ssi=%d, cr=%d, mod=%d\n",ssi,u8CodeRateStable,u8ModulationStable);
    }
    ssi = u8_ssi;
#else
    ssi = 0;
...
EN_MSB_1233C_LOCK_STATUS DTV_DVB_T_GetLockStatus(void)
{
    EN_MSB_1233C_LOCK_STATUS eLockStatus = E_MSB_1233C_DEMOD_CHECKING;
...
MAPI_BOOL DTV_GetPlpBitMap(MAPI_U8* u8PlpBitMap)
{
    ...
    if (u8_acc == 0)
    {
        ...
        DBG_DEMOD_MSB(printf("[msb1233c][SPLP with Common]Trigger!!!!!!!, plp_id=%d, group_id=%d, u8_index=%d,u8_mod=%d\n",u8_plp_id,u8_group_id,u8_index,u8_mod));
    }
...
MS_BOOL DTV_Get_Current_Plp_Id(MS_U8 *plp_id)
{
    ...
    {
        *plp_id = u8_plp_id;
        printf("[msb1233c][SPLP with Common]Trigger!!!!!!!, dspregid=%d, regid=%d\n",reg,u8_plp_id);
    }
    else
    {
        *plp_id = reg;
    ...
    DBG_DEMOD_MSB(printf("[msb1233c]DTV_Get_Current_Plp_Id, plpid=%d\n",*plp_id));
...
ST_MAPI_DEMODULATOR_FUN* DeviceDemodCreate(void)
{
    ...
    stMapiDemodulatorFun.DTV_GetLock = MSB1233C_Lock;
    if (bPower_init_en == MAPI_TRUE)
    {
        stMapiDemodulatorFun.bMSB123X_Ready = TRUE;
    }
