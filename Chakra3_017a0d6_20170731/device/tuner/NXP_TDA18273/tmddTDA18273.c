...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "tmddTDA18273DeInit(0x%08X)", tUnit);
...
)
{
    ...
    if(err == TM_OK)
    {
        ...
        pI2CMap_End = (UInt8*)&(pObj->I2CMap) + sizeof(TDA18273_I2C_Map_t) - 1;

        /* Save the values written in the Tuner */
        for (uCounter = 0; uCounter < uNbBytes; uCounter++)
        {
        #if 1 // Fix coverity
            if( pI2CMap <= pI2CMap_End )
            {
...
)
{
    ...
    if(err == TM_OK)
    {
        ...
        /* Write byte 0x06 */
        pObj->I2CMap.uBx06.Power_state_byte_2 = pObj->I2CMap.uBx06.Power_state_byte_2&0x03;
