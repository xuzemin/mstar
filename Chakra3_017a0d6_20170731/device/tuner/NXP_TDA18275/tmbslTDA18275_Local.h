 ...
 * \par Changelog
 *  -0.1.1 TK48:  AGC2_Vsync_Enable set with custom value  in HwInit() API
 *  -0.1.0 TK40: new API tmbslTDA18275_SetFineRF_Step()
 *                 Fine tune RF with input  parameter in Hz
 *     RFT222:
 *              check POR bit in separate I2C read outside burst mode
 *     
 *  -0.0.28 RFT202 : add WIMAX immunity option
 *     RFT206 : improve ACI N-4 & N-3  in ATSC
 *  -0.0.27 RFT208 : improve PLD measurement repeatability
 *  -0.0.26 fix PLD calc error   ( QAM6 & QAM8 RF freq 169 - 179 Mhz )
 *  -0.0.25 RFT187 : 
 *         Update VCO Current control  table vs ADC_Vtune ( T > 75° )
 *         Adapt new VCO rules choice criterias and margins calculations
 *  -0.0.25 RFT187 : 
 *         Update of Current control VCO table vs ADC_Vtune ( T > 75° )
 *         Fix Margin calculations for VCO low min vs WIFI frequency
 *  -0.0.24 RFT174 :  PLD improved with Standard dependant adjustements
...
#define TDA18275_MAJOR_VER 1  /* Minor protocol change - Specification change required */
#define TDA18275_MINOR_VER 1  /* Software update - No protocol change - No specification change required */
