...
//static IR_RegCfg   _tIrRegCfg;   //removed in t3 bring up
...
void USB_Init_Configure(void)
{
    ...
    //open port 1  // USB1_regbase: 0x103a00
    ...
    //Disable battery charge mode
    MDrv_WriteRegBit(0x123620+0x0C, DISABLE, 0x40);// [6]= reg_into_host_bc_sw_tri
    MDrv_WriteRegBit(0x123620+0x03, DISABLE, 0x40);// [6]= reg_host_bc_en
    MDrv_WriteRegBit(0x103A00+0x01, DISABLE, 0x40);//IREF_PDN=1¡¦b1. (utmi+0x01[6] )

    MDrv_USB_EnterPowerSaveMode_Port2();

    //open port 2  // USB2_regbase: 0x103900
    ...
    while( (MDrv_ReadByte(0x103900+0x3C) & 0x02) == 0)  // polling bit <1> (CA_END)
    {
        if (ii++>10000)
        {
            printf("Poll USB port2 CA_END timeout\n");
            break;
        }
    }
    ...
    //improve the efficiency of USB access MIU when system is busy
    MDrv_WriteRegBit(0x113900+0x81, ENABLE, 0xF);

//Disable battery charge mode
    MDrv_WriteRegBit(0x123640+0x0C, DISABLE, 0x40);// [6]= reg_into_host_bc_sw_tri
    MDrv_WriteRegBit(0x123640+0x03, DISABLE, 0x40);// [6]= reg_host_bc_en
    MDrv_WriteRegBit(0x103900+0x01, DISABLE, 0x40);//IREF_PDN=1!|b1. (utmi+0x01[6] )

    MDrv_USB_EnterPowerSaveMode_Port3();
