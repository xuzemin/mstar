...
//
// File Name: DrvAuTAS5706.c
// Description: TI I2S Audio Amplifier
// Revision History:
//
////////////////////////////////////////////////////////////////////////////////
#define DRV_TAS5706_C
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#include"drvIIC.h"
#include "GPIO.h"
#include "DevTAS5706.h"
#include "MApp_GlobalVar.h"
#include "msAPI_Timer.h"

#if (DIGITAL_I2S_SELECT==AUDIO_I2S_TAS5706)

#define TAS5706_ADDR     0x36
#define TAS5706_BUSID     (((U16) E_I2C_BUS_SYS << 8) | TAS5706_ADDR)

static TAS5706_REGMAP				sm_sRegImage;
...
static BOOLEAN si_dvTAS5706RegsRead(void)
{
    U8* pu8Buf;
    U8  cSubAddr[1];
    cSubAddr[0] = sm_sRegImage.ucAddr;
    pu8Buf = sm_sRegImage.sDatas;
    return MDrv_IIC_ReadBytes(TAS5706_BUSID, 1, cSubAddr, sm_sRegImage.ucNumber, pu8Buf);
}



static BOOLEAN si_dvTAS5706RegsWrite(void)
{
    U8 *pu8Buf;
    U8  cSubAddr[1];
    cSubAddr[0] = sm_sRegImage.ucAddr;
    pu8Buf = sm_sRegImage.sDatas;
    return MDrv_IIC_WriteBytes(TAS5706_BUSID,1,cSubAddr,sm_sRegImage.ucNumber,pu8Buf);
...
code TAS5706_REGMAP DrvTas5706InitTbl1[] =
{
...
BOOLEAN Drv_TAS5706_WRITE_DATAS(U8 ucSubAddr, U8 ucCnt, U8* pBufs )
{
...
BOOLEAN Drv_TAS5706_READ_DATAS(U8 ucSubAddr, U8 ucCnt, U8* pBufs )
{
    BOOLEAN bResult = FALSE;
...
void drvTAS5706_Subwoof(void)
{
    sm_sRegImage.ucAddr=0x0D;
    sm_sRegImage.ucNumber=1;
    sm_sRegImage.sDatas[0]=0x35;
    si_dvTAS5706RegsWrite();
}
void drvTAS5706_SubwoofValue(U8 Subwoof)
{
    sm_sRegImage.ucAddr=0x0D;
    sm_sRegImage.ucNumber=1;
    sm_sRegImage.sDatas[0]=Subwoof;
    si_dvTAS5706RegsWrite();
}
/*
void drvTAS5706_MuteSubwoof(BOOLEAN bMute)
{
    U8* pu8Buf;
    U8 u8Temp;

    MDrv_IIC_ReadBytes(TAS5706_BUSID,0x06,1,pu8Buf);
    u8Temp = *pu8Buf;
...
void drvTAS5706_RegInit(void)
{
    ...
    while(1)
    {
    	if(DrvTas5706InitTbl1[iu8Loop_i].ucAddr == __ENDTBL__)
		break;
        sm_sRegImage.ucAddr = DrvTas5706InitTbl1[iu8Loop_i].ucAddr;
        sm_sRegImage.ucNumber = DrvTas5706InitTbl1[iu8Loop_i].ucNumber;
        memcpy((U8*)&sm_sRegImage.sDatas[0],
            (U8*)(&DrvTas5706InitTbl1[iu8Loop_i].sDatas[0]),
            sm_sRegImage.ucNumber);
        bResult=si_dvTAS5706RegsWrite();
	 if(bResult)
	 {
	        TAS_MSG(printf("\r \n dai Write 5706 Success!\n"));
	}
	else
    	{
	        TAS_MSG(printf("\r \n dai Write 5706 Fail!\n"));
...
void drvTAS5706_Mute(BOOLEAN bmute)
{
    BOOLEAN bResult = FALSE;
...
void drvTAS5706_SourceVolSel(INPUT_SOURCE_TYPE_t enInputSourceType)
{
