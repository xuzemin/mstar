...
void devTunerSetPLLData(WORD wPLLData, RFBAND eBand)
{
    ...
    cTunerData[2] = 0xCE;//0x88|DIVIDER_RATIO;

    if(eBand == E_RFBAND_VHF_LOW)
    {
        cTunerData[3] = 0x01|_u8Antenna;
    }
    else if(eBand == E_RFBAND_VHF_HIGH)
    {
        cTunerData[3] = 0x02|_u8Antenna;
    }
    else
    {
        cTunerData[3] = 0x04|_u8Antenna;
    }
    cTunerData[4] = 0xDE;//Enable Exteral AGC(T2T1T0=011)
...
void devDigitalTuner_SetFreq ( double Freq, RF_CHANNEL_BANDWIDTH eBandWidth)
{
    ...
    if(Freq < 160)
    {
        if(eBandWidth == E_RF_CH_BAND_7MHz)
            CONFIG[3] = 0x01|_u8Antenna;
        else if(eBandWidth == E_RF_CH_BAND_8MHz)
            CONFIG[3] = 0x11|_u8Antenna;
    }
    else if(Freq < 445)
    {
        if(eBandWidth == E_RF_CH_BAND_7MHz)
            CONFIG[3] = 0x02|_u8Antenna;
        else if(eBandWidth == E_RF_CH_BAND_8MHz)
            CONFIG[3] = 0x12|_u8Antenna;
    }
    else
    {
        if(eBandWidth == E_RF_CH_BAND_7MHz)
            CONFIG[3] = 0x04|_u8Antenna;
        else if(eBandWidth == E_RF_CH_BAND_8MHz)
            CONFIG[3] = 0x14|_u8Antenna;
    }
    CONFIG[4] = 0xDC;//0x9C;
    CONFIG[5] = 0xA0; //CB2

    //printf("Tuner: %x, %x, %x, %x, %x\n", CONFIG[0], CONFIG[1], CONFIG[2], CONFIG[3], CONFIG[4]);
