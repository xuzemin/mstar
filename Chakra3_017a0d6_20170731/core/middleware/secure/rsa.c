...
static int _QhatTooBig(DIGIT_T qhat, DIGIT_T rhat,
...
static U32 t1[RSA_KEY_DIGI_LEN*2];
static U32 t2[RSA_KEY_DIGI_LEN*2];
static U32 t3[RSA_KEY_DIGI_LEN*2];
static U32 tm[RSA_KEY_DIGI_LEN];
static U32 y[RSA_KEY_DIGI_LEN];

int mpModExp(DIGIT_T yout[], const DIGIT_T x[],
            const DIGIT_T e[], const DIGIT_T m[], int ndigits)
...
U32 RSA_Test(void)
{
    ...
    mpConvFromOctets(n, RSA_KEY_DIGI_LEN, nn, sizeof(nn));
    mpConvFromOctets(e, RSA_KEY_DIGI_LEN, ee, sizeof(ee));
    mpConvFromOctets(m, RSA_KEY_DIGI_LEN, mm, sizeof(mm));
    mpConvFromOctets(s1, RSA_KEY_DIGI_LEN, ss, sizeof(ss));

    /* Verify, i.e. Decrypt with public key m' = s^e mod n */
    mpModExp(m1, s1, e, n, RSA_KEY_DIGI_LEN);

    /* Check that we got back where we started */
    if (!mpEqual(m1, m, RSA_KEY_DIGI_LEN))
    {
        return 1;
    }
    else
