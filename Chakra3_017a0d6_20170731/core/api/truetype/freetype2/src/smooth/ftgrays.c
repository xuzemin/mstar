...
#undef  FT_ERR_PREFIX
#define FT_ERR_PREFIX  FT_Err_  //Smooth_Err_
...
#define ErrRaster_Memory_Overflow   FT_Err_Memory_Overflow
...
static BOOLEAN bRenderEdgePatch = FALSE;
...
void ft_gray_RenderEdgePatch(BOOLEAN bEnable)
{
    bRenderEdgePatch = bEnable;
}
                    ...
                    gray_PWorker    worker )
  {
    ...
    for ( ; count > 0; count--, spans++ )
    {
      ...
      if ( coverage )
      {
         ...
         * function call.
         */
          if ( (bRenderEdgePatch) && ( (*worker).max_ex == ( spans->x + spans->len ) ) )
          {
                FT_Span *preSpans = (FT_Span *)spans;

                if ( preSpans != orgSpans )
                {                
                    preSpans--;
                    coverage = preSpans->coverage;
                }
          }
...
#define SCALED( x )  ( ( (x) << shift ) - delta )
    ...
    for ( n = 0; n < num_bands; n++, min = max )
    {
      ...
      while ( band >= bands )
      {
...

        {
          ...
          cells_max = (PCell)((void *)( (char*)ras.buffer + cell_end ));
          ras.cells = (PCell)((void *)( (char*)ras.buffer + cell_start ));
                     ...
                     long       pool_size )
  {
    ...
    if ( raster )
    {
      ...
      {
        gray_PWorker  worker = (gray_PWorker)((void *)pool_base);
