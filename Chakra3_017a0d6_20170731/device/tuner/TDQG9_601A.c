...
#define DIVEDER_RATIO_142_857   0x01
#define DIVIDER_RATIO_62_5KHz   0x03
#define DIVIDER_RATIO_50KHz     0x02
...
static U8 _u8Antenna;

U8 ucStatus; // tuner status

void devTunerSetAntenna(BOOLEAN bIsCATV)
{
    if(bIsCATV)
        _u8Antenna = 0x01;
    else
...
void devTunerSetPLLData(WORD wPLLData, RFBAND eBand)
{
    ...
    #else
    cTunerData[2] = 0x80|DIVIDER_RATIO|IF_OUT_LEVEL_MINUS_3_dBM;
    #endif

    if(eBand == E_RFBAND_VHF_LOW)
    {
        cTunerData[3] = 0x64|_u8Antenna;
    }
    else if(eBand == E_RFBAND_VHF_HIGH)
    {
        if ( u16Freq <= 398 )
        {
            cTunerData[3] = 0x66|_u8Antenna;
        }
        else
        {
            cTunerData[3] = 0xA6|_u8Antenna;
        }
    }
    else
    {
        if ( u16Freq <= 782 )
        {
            cTunerData[3] = 0xAC|_u8Antenna;
        }
        else
        {
            cTunerData[3] = 0xEC|_u8Antenna;
...
void devDigitalTuner_SetFreq ( double Freq, RF_CHANNEL_BANDWIDTH eBandWidth)
{
    ...
    CONFIG[2] = 0xA0;//0x88; // CB1
     //CONFIG[3]  = 0x00;
    if(Freq < 153)
    {
        if(eBandWidth == E_RF_CH_BAND_7MHz)
            CONFIG[3] = 0x60|_u8Antenna;
        else if(eBandWidth == E_RF_CH_BAND_8MHz)
            CONFIG[3] = 0x64|_u8Antenna;
    }
    else if(Freq < 398)
    {
        if(eBandWidth == E_RF_CH_BAND_7MHz)
            CONFIG[3] = 0x62|_u8Antenna;
        else if(eBandWidth == E_RF_CH_BAND_8MHz)
            CONFIG[3] = 0x66|_u8Antenna;
    }
    else if(Freq < 430)
    {
        if(eBandWidth == E_RF_CH_BAND_7MHz)
            CONFIG[3] = 0xA2|_u8Antenna;
        else if(eBandWidth == E_RF_CH_BAND_8MHz)
            CONFIG[3] = 0xA6|_u8Antenna;
    }
    else if(Freq < 782)
    {
        if(eBandWidth == E_RF_CH_BAND_7MHz)
            CONFIG[3] = 0xA8|_u8Antenna;
        else if(eBandWidth == E_RF_CH_BAND_8MHz)
            CONFIG[3] = 0xAC|_u8Antenna;
    }
    else
    {
        if(eBandWidth == E_RF_CH_BAND_7MHz)
            CONFIG[3] = 0xE8|_u8Antenna;
        else if(eBandWidth == E_RF_CH_BAND_8MHz)
            CONFIG[3] = 0xEC|_u8Antenna;
    }
